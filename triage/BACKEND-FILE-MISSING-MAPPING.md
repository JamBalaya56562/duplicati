...
**GoogleDrive のキャッシュには別のバグがありました。**捨てる側（この族）は元から正しかった
一方、**埋める側**が壊れていて、`Array.Resize(ref lst, …)` が新しい配列をローカルにしか書かない
ため、同名の2件目以降がキャッシュから落ちていました。2026-09-08 に
[#7282](https://github.com/duplicati/duplicati/pull/7282) で修正済みです。**この族とは独立**で、
(a) の判定は変わりません。
