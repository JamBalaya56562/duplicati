...
> **更新履歴 (2026-08-15)**: 掲載中の深掘り Issue **57 件すべて**の現在の state を GitHub API で照会し、クローズ済みの 3 件を削除しました — #2487「scheduled Jobs are not updated after Change of weekday」（2026-07-22 close、§3.2 表・§4.3 詳細・§5 付録）、#2612「Clicking Windows 10 notification does not open Duplicati」（2026-08-11 close、§3.4 表・§4 詳細・§5 付録）、#2838「Support native environment variable syntax」（2026-07-22 close、§3.4 表・§4 詳細・§5 付録）。削除に伴い §3 見出し（66→54 件）・§3.1（35→34 件）・§3.2（5→4 件）・§3.4（17→15 件）を実数に合わせました。**upstream の open 総数は本日時点で 632 件**（初版時 647 件）ですが、§1 のサマリー統計（バケット別内訳・ラベル分布・年代分布）と §2 の方法論記述は全件再分類が必要になるため、初版スナップショットのまま据え置きです。

> **更新履歴 (2026-08-18)**: [#3853](https://github.com/duplicati/duplicati/issues/3853)「EndTime: 0001-01-01T00:00:00 after restore」を削除しました（2026-08-18 に COMPLETED でクローズ、[#7182](https://github.com/duplicati/duplicati/pull/7182) による修正）。**本レポートの当初の判定は誤っていました。** §3.1 は「シリアライザで除外済み」としてFIXED と記載していましたが、除外があったのは `BasicResults.ToString()` のテキスト出力側だけで、JSON 側は素通しでした。報告書を読むだけで済ませず、実際に結果を捕獲して確かめる必要があった例です。削除に伴い §3 見出し（54→53 件）・§3.1（34→33 件）を実数に合わせました。

> **更新履歴 (2026-08-21)**: 解決した2件を反映しました。[#7127](https://github.com/duplicati/duplicati/issues/7127)「Waiting for upload to finish で停止」と [#3672](https://github.com/duplicati/duplicati/issues/3672)「restore 中の Backend quota 誤報」が、いずれも 2026-08-21 に COMPLETED でクローズされました（[#7181](https://github.com/duplicati/duplicati/pull/7181) と [#7184](https://github.com/duplicati/duplicati/pull/7184)）。#3672 は §5 付録から削除し、[#3546](https://github.com/duplicati/duplicati/issues/3546)（修正は [#7185](https://github.com/duplicati/duplicati/pull/7185) で提出済み・マージ待ち）には状態を注記しました。**古い Issue は「議論が止まっているから死んでいる」とは限りません** — #3546 も #3672 も7年動いていませんでしたが、どちらも現行 master で再現し、原因も小さく、修正できました。動いていないのは、調べた人がいなかったからです。

> **更新履歴 (2026-08-22)**: [#4880](https://github.com/duplicati/duplicati/issues/4880)「No "+" allowed in path of WebDAV destinations」が 2026-08-22 に COMPLETED でクローズされました（[#7197](https://github.com/duplicati/duplicati/pull/7197) による修正）。§5 付録から削除しています。**【2026-08-27 訂正】この「修正」は誤りでした。詳細は下記 2026-08-27 の項を参照。**原因は `UrlEncoding.UrlDecode` が `+` を空白として解いていたことで、これは `application/x-www-form-urlencoded`（クエリ文字列）の規則であって URL のパスでは誤りでした。**正しい挙動は既に同じファイルの5箇所に書かれていました** — 応答パース側だけが `.Replace("+", "%2B")` で守られており、入口が直し忘れられていた形です。**「一部だけ直っている」箇所は、残りが直っていない証拠として読めます。**あわせて [#3536](https://github.com/duplicati/duplicati/issues/3536)「Recursive Folders (due to symlinks?) in Linux」に**着手は非推奨**と注記しました。22件のコメントが付いた古いバグですが、報告の中心にあった symlink 説はメンテナ自身の最小再現で否定されており、現行 master で再現する手順が誰からも示されていません。**再現できない以上、直したかどうかを確かめる方法がありません。**さらに §5 付録の小見出し4件の宣言件数が過去の削除で実数とずれていたため実数に合わせました（2017年以前 24→23、reproduced 11→9、minor change 31→30、その他 bug 68→66、付録合計 630→624）。

> **更新履歴 (2026-08-27)**: マージされた2件と、**過去の記述の訂正**です。
>
> **訂正 — [#4880](https://github.com/duplicati/duplicati/issues/4880) は #7197 では直っていませんでした。**`+` → 空白は `NormalizePath` ではなく **`RelaxedUri` のパース内**で起きており、`RelaxedUri.Path` は復号済みの文字列を返すため、`NormalizePath` が受け取る時点で `+` は消えていました。#7197 に付けたテストは `NormalizePath` に**生の未復号文字列を直接渡していた**ので、コンストラクタが実際に渡す値ではないものを測っていました。赤→緑は本物でしたが、間違った入力の赤でした。Issue は COMPLETED のまま閉じられていますが**バグは残っており**、本当の修正は [#7200](https://github.com/duplicati/duplicati/pull/7200)（WebDAV を `System.Uri` で解析）で、2026-08-27 時点で**レビュー待ち**です。
>
> **教訓**: シームは**実際の呼び出し元が渡す値**を受け取らなければ、赤→緑は何も証明しません。外部へのリクエストで終わる修正は、**送信されるリクエストそのもの**を検証すること。「赤が出た」だけでなく**赤の内容が報告された症状と一致するか**まで見ること。
>
> **マージ**: [#7202](https://github.com/duplicati/duplicati/pull/7202)（Dropbox のパスを二重復号していた。名前に `%` を含むフォルダに到達できなかった）と、[#7132](https://github.com/duplicati/duplicati/pull/7132)（欠けた dlist を差し替えると fileset の時刻が前へずれて版番号が振り直される。Issue 番号は無く、#7128 の CI 失敗調査から見つけたもの）。どちらも §5 付録の Issue には対応しません。
>
> URL の復号・符号化に関する一連の調査は [BACKEND-URL-HANDLING.md](BACKEND-URL-HANDLING.md) にまとめました。

> **更新履歴 (2026-08-27, 2)**: `backendtests.yml` の**赤は flaky だけではありませんでした**。直近25実行のうち失敗した8実行、**失敗ジョブ10件すべてのログ**を読んだところ、純粋な環境起因は**2件だけ**で、残り8件は再現性のある欠陥でした。原因は**2つ**しかありません。
>
> **1件目は利用者に影響する本物のバグ** — Drime の削除が一度タイムアウトすると恒久的に失敗します。キャッシュが失敗時に無効化されないためリトライが死んだ entry id を送り続け、その応答が `FileMissingException` にマップされないため `BackendManager.DeleteOperation` の既存の復旧に乗れません。Drime は上書き非対応で `PutAsync` が先に削除するので、アップロードも巻き添えになります。修正は [#7221](https://github.com/duplicati/duplicati/pull/7221)。
>
> **2件目は BackendTester 自身の欠陥** — 7月に入れた `DeleteListedFileAsync` を本体の削除ループが使っておらず、開始時の空チェックだけがリトライを持ちません。pCloud・CloudStack・B2 の6件がこれです。修正は [#7222](https://github.com/duplicati/duplicati/pull/7222)。**自分が入れた修正の「使われていない側」を疑うこと** — `DeleteListedFileAsync` は7月に作ったのに呼び出しが1箇所しかありませんでした。
>
> **「live backend test が赤 = flaky」と決めつけないこと。** 10件の赤のうち8件は2つの欠陥に帰着し、残る2件が環境起因でした。

> **更新履歴 (2026-08-28)**: [#7221](https://github.com/duplicati/duplicati/pull/7221)（Drime の削除がタイムアウト後に恒久的に失敗する）が**マージ**されました。§5 付録の Issue には対応しません。
>
> **訂正 — `DisruptionTests.TestMultiExceptionOnDlistAsync` は macOS 限定ではありませんでした。**#7221 の CI で **ubuntu** が落ちました。差分は `DrimeCloud/` だけで、`DisruptionTests` は `file://` を使うため到達しません。同一実行の windows と macOS は緑、数分後の #7222 は同じ ubuntu ジョブが緑。既知の発生は windows / macOS / ubuntu の3 OS にまたがります。**フレークだと判断するのに OS は根拠になりません** — 差分がそのテストに到達し得るか、同じケースが無関係な PR でも落ちているか、で判断すること。
>
> **[#7177](https://github.com/duplicati/duplicati/pull/7177) の投資が回収されました。**8月に足した診断のおかげで、今回の失敗が版一覧つきで届き、[DLIST-FLAKE-INVESTIGATION.md](DLIST-FLAKE-INVESTIGATION.md) の1つ目の署名と一致することがその場で分かりました。
>
> **教訓 — 調べ直す前に triage を読むこと。** この日、`doUpload` の判断点も、「最初の中断回が投げたときだけ落ちる」という相関も、合成 filelist の連鎖も、**すべてソースから再導出しました。全部あのファイルに書いてありました。**数時間を費やして新しい事実は3つだけ（OS の訂正、Linux ローカル環境、負荷実験の否定的結果）です。**`triage/` にファイルがあるのは、同じ道を二度歩かないためです。**

> **更新履歴 (2026-08-29)**: URL 処理の系列が2件**マージ**されました — [#7201](https://github.com/duplicati/duplicati/pull/7201)（SharePoint を `System.Uri` で解析）と [#7204](https://github.com/duplicati/duplicati/pull/7204)（OneDrive のフォルダ名を URL に符号化して載せる）。どちらも §5 付録の Issue には対応しません。
>
> **[#4880](https://github.com/duplicati/duplicati/issues/4880) はまだ直っていません。**本当の修正は [#7200](https://github.com/duplicati/duplicati/pull/7200)（WebDAV）で、これが URL 系列で**唯一の未マージ**です。Issue は 2026-08-22 に COMPLETED で閉じられたままです。
>
> **#7204 はメンテナの実装と統合されました。** kenkendk が同じ8箇所を [#7219](https://github.com/duplicati/duplicati/pull/7219) で別目的（ドライブ直下の参照）から集約しており、統合後は符号化が `BuildRootUrl` の中に入りました。**こちらの方が元の実装より良く**、`ListAsync(path)` などの3箇所も自動的に符号化されます。設計上の要（`RootPath` は復号済みのまま、URL に載せる時だけ符号化）は保たれ、`OneDriveUrlTests` 12件は統合後も緑でした。詳細は [BACKEND-URL-HANDLING.md](BACKEND-URL-HANDLING.md)。
>
> **教訓 — 同じ場所を触っている未マージ PR を先に見ること。** #7219 は #7204 と8箇所すべてが重なっていました。目的が違ったので衝突ではなく統合で済みましたが、気づいたのはメンテナがコメントを付けた後でした。

> **更新履歴 (2026-08-29, 2)**: 同じ日に赤が2件出て、**片方は本物、片方はフレーク**でした。見分けたのは OS の数です。
>
> **本物 — [#7185](https://github.com/duplicati/duplicati/pull/7185) が 3 OS すべてで失敗。**版ラベル機能（2026-08-22 マージ）が `RecreateDatabaseHandler.cs:275` に `filelists.First().File.Name` を入れていました。master ではこの行に**到達しません** — #7185 が取り除く「filelist が無ければ throw」がその手前に立っているからです。取り除いた結果、空のリストで到達して `Sequence contains no elements` になりました。**master のバグではなく、私の変更が既存コードの前提を外した形です。**`newestFilelistName` は filelist を回すループ内の比較にしか使われず、ゼロ件ならループが回らず `remoteLabels` は null のまま `ApplyLabelsFromFilelistAsync` が即 return するので、`FirstOrDefault()?.File.Name` の1行で済みました。rebase して修正済み。
>
> **フレーク — [#7173](https://github.com/duplicati/duplicati/pull/7173) が windows のみで失敗。**`DisruptionTests.TestMultiExceptionOnDlistAsync(False,False,3,True)` で、[DLIST-FLAKE-INVESTIGATION.md](DLIST-FLAKE-INVESTIGATION.md) の**2つ目の署名と一字一句一致**（1 対 2、0 added / 0 modified、full 版が2つちょうど1秒差）。#7173 の差分は `LocalTestDatabase` への `CREATE INDEX` 追加だけで、どの fileset が存在するかを変えようがありません。これで両方の署名が上流で観測され、**3 OS すべて × 両方の署名**にまたがることが確定しました。
>
> **教訓 — 落ちた OS の数を最初に見ること。** 3 OS で同じテストが落ちるなら、それは差分が壊しています。1 OS だけなら、**差分がそのテストに到達し得るか**と**署名が既知のものと一致するか**を確かめる。この2件は同じ日に、同じ「ユニットテストが赤」という見た目で来ました。

> **更新履歴 (2026-08-29, 3)**: 3件**マージ**されました — [#7173](https://github.com/duplicati/duplicati/pull/7173)（検証用一時テーブルへの索引）、[#7222](https://github.com/duplicati/duplicati/pull/7222)（BackendTester が「もう無い」を失敗として扱う件）、[#7233](https://github.com/duplicati/duplicati/pull/7233)（中断されたバックアップに何をしたか言わせる診断）。いずれも §5 付録の Issue には対応しません。
>
> **backendtests の調査は完了しました。** #7221（08-28）と #7222（08-29）で原因2つとも修正済みです。調べた10件の赤のうち8件がこの2つに帰着し、残る2件が環境起因でした。**完了したので調査文書は削除しました** — 経緯は2つの PR の本文に残っています。
>
> **#7233 で診断が上流に入りました。** これで `DisruptionTests` の1つ目の署名は、次に落ちた1回で「どの中断回が、なぜ上げたか」が分かります。**準備は終わり、あとは発生を待つだけ**です。2つ目の署名は対象外で、そちらは上げたこと自体が正常な経路です（[DLIST-FLAKE-INVESTIGATION.md](DLIST-FLAKE-INVESTIGATION.md)）。
>
> **README の死んだバッジ2件**（Coveralls と Travis）を [#7237](https://github.com/duplicati/duplicati/pull/7237) で削除に出しました。Coveralls は3つの README 以外に一切参照が無く、0% は測定結果ではなく測定の不在でした。Travis は `.travis.yml` すら無く、**現状の GitHub Actions では代替できません** — 全ワークフローが `[pull_request, workflow_dispatch]` で master への push で何も動かないため、バッジが映す既定ブランチの実行が存在しないからです（API で確認: `?branch=master` は2件のみ、どちらも2025年9月の失敗）。`push` トリガーを足すかは CI の構成判断なのでメンテナに委ねました。
>
> なお **LICENSE が GitHub で "Other" になるのは正常**です。`proprietary/` 配下がMIT ではない旨の前置き100語（全体の38%）が入っているため licensee が一致判定できません。`proprietary/` は実在し、その LICENSE はサブスクリプション契約を要求する別物なので、**前置きを消して表示を直すのは実態を単純化しすぎ**です。触りません。

> **更新履歴 (2026-08-29, 4)**: [#7237](https://github.com/duplicati/duplicati/pull/7237)（死んだバッジ2件の削除）と [#7238](https://github.com/duplicati/duplicati/pull/7238)（`--synchronous-upload` を非推奨に）が**マージ**されました。
>
> **[#7203](https://github.com/duplicati/duplicati/pull/7203) はクローズしました** — kenkendk が [#7226](https://github.com/duplicati/duplicati/pull/7226)「Ensure empty metadata exists」で同じ問題に対処し、**#7203 の実装とテストが丸ごと master に入った**ためです。メソッド名（`FindEmptyMetadataBlocksetIdAsync` / `StoreEmptyMetadataAsync` / `AddEmptyMetadataBlocksetAsync`）も、書いたコメントの文面も、`ReplacementMetadataTests` の3ケースも同一です。
>
> **「たぶん直った」を読んで済ませないこと。** メンテナから「同種の問題に対処する書き換えをマージした。たぶん直っていると思うが読みを聞きたい」と訊かれたので、**master をビルドして該当テストを実行しました（3/3 緑）**。バグが在れば `TheReplacementIsEmptyMetadataAndNotAnotherBlocksetAsync` が落ちるので、これは確認であって推測ではありません。あわせて**一番の難所が保たれているか**も確かめました — SQLite の rowid 再利用対策（`AddEmptyMetadataBlocksetAsync` で ID を明示する）は `LocalListBrokenFilesDatabase.cs:466` に説明コメントごと残っていました。落ちていれば追加 PR を出す必要がありましたが、不要でした。**調査文書 `REPLACEMENT-METADATA-INVESTIGATION.md` は完了したので削除しました** — 再利用価値のある知見（rowid 再利用の罠）は、出荷コード中のコメントという一番読まれる場所に残っています。

> **更新履歴 (2026-09-01)**: マージ2件と、**backendtests の3つ目の原因**です。
>
> **マージ**: [#7240](https://github.com/duplicati/duplicati/pull/7240)（Filejump）と [#7241](https://github.com/duplicati/duplicati/pull/7241)（Filen）。どちらもコメント無し・無改変でマージされ、master 上で実装とテストの両方を確認しました。これで**名前→id のキャッシュを持つ6バックエンドの掃討は完了**です。
>
> | バックエンド | 削除が失敗したときにキャッシュを捨てるか |
> |---|---|
> | Box / GoogleDrive / Mega | 元から有り |
> | Drime | [#7221](https://github.com/duplicati/duplicati/pull/7221) |
> | Filejump | [#7240](https://github.com/duplicati/duplicati/pull/7240) |
> | Filen | [#7241](https://github.com/duplicati/duplicati/pull/7241) |
>
> **訂正 — 「backendtests の調査は完了しました」（2026-08-29, 3）は、原因が2つで終わりという意味ではありませんでした。**あれは**当時読んだ赤10件**についての結論です。2026-08-31、**無関係な3つのブランチ**（[#7243](https://github.com/duplicati/duplicati/pull/7243) / [#7244](https://github.com/duplicati/duplicati/pull/7244) / [#7245](https://github.com/duplicati/duplicati/pull/7245)）で **GCS Tests が3 OS すべて**同じ形で落ちました。3つ目の原因で、フレークではありません。
>
> ```
...
> ```
>
> 削除の1回目が答えを返さず、リトライが「もう無い」を受け取る — **#7221 / #7240 / #7241 と同じ族**です。**受け皿は既に在りました**: `BackendExtensions.cs:91` の `catch (FileMissingException)` は [#7109](https://github.com/duplicati/duplicati/pull/7109) で私が入れたものです。乗らなかったのは **`GoogleCloudStorage.DeleteAsync` が 404 を `FileMissingException` に写さないから**でした。修正は kenkendk の [#7245](https://github.com/duplicati/duplicati/pull/7245) で、**こちらから出す必要はありません**。
>
> **族には独立した半分が2つあり、私が掃いたのは片方だけでした。**(a) 失敗時にキャッシュを捨てる、(b)「もう無い」を `FileMissingException` に写す。**6件の掃討は「キャッシュを持つか」で絞った**ので、キャッシュを持たない GCS は最初から範囲外でした。#7241 では (b) を「エラー本文を測れていない」として意図的に落としましたが、**区別が粗すぎました** — **HTTP 404 は測る必要がありません**（契約が決まっている）。測れないのは Drime の `entryIds` のような **API 独自の本文だけ**です。
>
> **GCS は「一部だけ直っている」の実例でした。**同じファイルの `GetObjectLockUntilAsync` / `SetObjectLockUntilAsync` / `GetAsync` の3箇所は 404 → `FileMissingException` を持ち、`DeleteAsync` と `RenameAsync` だけが持ちません。2026-08-22 に書いた「**一部だけ直っている箇所は、残りが直っていない証拠として読める**」がそのまま当たりました。
>
> **#7245 でも GCS Tests はまだ緑ではありません。**失敗が次の段へ動き、`PutAsync` の直後の読み戻しが「無い」と言われています。**私には測る手段がありません**（GCS の認証情報が無い）。メンテナが対応中なので**先回りしないこと**。方法と暫定表は [BACKEND-FILE-MISSING-MAPPING.md](BACKEND-FILE-MISSING-MAPPING.md) に分けました。**素朴な grep では判定できません** — ファイル内に `FileMissingException` が在るかで見ると GCS は「有り」と出ます。
>
> **`DisruptionTests` の発生はまだありません。**#7233 のマージ（08-29）以降、`tests.yml` の直近30実行のうち赤は3件で、いずれも**3 OS 揃って**落ちています（IPC の machineId 2件、metadata repair 1件）。すべてメンテナ自身の差分が壊したもので、**待ちの状態は変わりません**。
>
> **未マージは3件**: [#7200](https://github.com/duplicati/duplicati/pull/7200)（WebDAV。[#4880](https://github.com/duplicati/duplicati/issues/4880) の本当の修正で、Issue は COMPLETED で閉じられたまま）、[#7185](https://github.com/duplicati/duplicati/pull/7185)、[#7180](https://github.com/duplicati/duplicati/pull/7180)。08-31 20:15 に3件とも CI を再実行中です。

> **更新履歴 (2026-09-01, 2)**: マージ2件と、**私の見立ての訂正**です。
>
> **マージ**: [#7180](https://github.com/duplicati/duplicati/pull/7180)（fileset 整合性チェックの二重 DISTINCT）と [#7246](https://github.com/duplicati/duplicati/pull/7246)（ドライブ文字だけのソースが作業ディレクトリになる）。
>
> **#7180 は1件で終わりませんでした。** kenkendk が [#7247](https://github.com/duplicati/duplicati/pull/7247)「Remove redundant distinct」で同じ知見を `LocalDeleteDatabase` / `LocalPurgeDatabase` に広げて**マージ**しています。本文に "This is a follow up to the issue discovered in #7180" と書かれています。**1箇所の指摘がメンテナ側の掃討につながった形**です。
>
> **訂正 — GCS の2段目の失敗について、私の見立ては誤りでした。**[BACKEND-FILE-MISSING-MAPPING.md](BACKEND-FILE-MISSING-MAPPING.md) に「`PutAsync` の直後に読めない＝書いた直後のファイルがすぐ参照できるとは限らない」と書きましたが、実際の原因は **`RelaxedUri` によるパスの二重符号化**でした。[#7249](https://github.com/duplicati/duplicati/pull/7249) の説明によれば、GCS には `folder/file` を `folder%2Ffile` として送る必要があるところ、再符号化で `folder%252Ffile` になり、**リテラルの `%2F` を含む名前で書かれていた**。書いた直後に読めなかったのは**タイミングではなく、別の名前で書かれていたから**です。
>
> **教訓 — 測れないものを「たぶんこう」で埋めないこと。** あの節には「私には測る手段がありません」と正しく書きましたが、そのすぐ隣に既知の現象（#7109 のコメント）を根拠として並べたため、**測っていない推測が測った事実のように読める形**になっていました。並べるなら、どちらが測定でどちらが推測かを行単位で分けること。
>
> **[#7245](https://github.com/duplicati/duplicati/pull/7245) はクローズされ、[#7249](https://github.com/duplicati/duplicati/pull/7249) に置き換わりました。**404 → `FileMissingException` のマッピングもそちらに含まれています。2026-09-01 11時時点で backendtests は**実行中**で、まだ緑を見ていません。**先回りしない**方針は変わりません。
>
> **[#7248](https://github.com/duplicati/duplicati/pull/7248) が DBus バインディングを再生成しています**（+1192/-798、"This fixes a few warnings"）。[DBUS-CODEGEN-REGENERATION.md](DBUS-CODEGEN-REGENERATION.md) の結論と一致する内容です。**マージされれば「警告3件」というビルドの基準が変わります** — 次に警告を比べるときは master を取り直すこと。
>
> **`DisruptionTests` の発生はまだありません。**[#7233](https://github.com/duplicati/duplicati/pull/7233) のマージから3日、直近の `tests.yml` に赤はありません。
>
> **未マージは2件**: [#7200](https://github.com/duplicati/duplicati/pull/7200)（WebDAV。[#4880](https://github.com/duplicati/duplicati/issues/4880) の本当の修正）と [#7185](https://github.com/duplicati/duplicati/pull/7185)。

> **更新履歴 (2026-09-04)**: **未マージがゼロになりました。**
>
> **マージ3件**: [#7260](https://github.com/duplicati/duplicati/pull/7260)（OpenStack の削除）、[#7185](https://github.com/duplicati/duplicati/pull/7185)（未完了の初回バックアップからの DB 再作成）、[#7200](https://github.com/duplicati/duplicati/pull/7200)（WebDAV を `System.Uri` で解析）。いずれも 09-04、**コメント無し・無改変**。master を取得して6つのテストファイルごと入っていることを確認しました。
>
> **[#3546](https://github.com/duplicati/duplicati/issues/3546) が COMPLETED でクローズ**されました（#7185 による）。2026-08-21 の項に「古い Issue は議論が止まっているから死んでいるとは限らない」と書いた、その7年動いていなかった Issue です。
>
> **[#4880](https://github.com/duplicati/duplicati/issues/4880) がやっと決着しました。**08-22 に #7197 を根拠にクローズされ、08-27 に「その根拠は誤りでバグは残っている」と訂正した件です。**本当の修正である #7200 が 09-04 に入りました。**Issue は閉じたまま中身が後から直った形で、URL 復号・符号化の系列はこれで**全件マージ済み**です（[BACKEND-URL-HANDLING.md](BACKEND-URL-HANDLING.md)）。
>
> **ビルド警告の基準は 0 件で確定**しました。[#7248](https://github.com/duplicati/duplicati/pull/7248) の DBus 再生成後、変更前後の全ビルド（`--no-incremental`）で 0/0 を確認しています。前回「次に比べるときは master を取り直すこと」と書いた宿題は済みです。
>
> **新しい問題 — master の Windows ユニットテストが赤です。**
>
> ```
...
> ```
>
> **誰の PR のせいでもありません** — **差分が到達し得ないブランチでも落ちています**（dependabot が npm 依存だけを上げたブランチ 09-02 19:19、および私の WebDAV PR 09-04 07:56。後者の差分は `Library/Backend/WEBDAV/` のみ）。エラー本文が**空**（`Error: ` の後ろが無い）のも気になる点です。出はじめは 09-02 の 13:58（緑）と 14:30（赤）の間で、**native VSS の一連の変更が入った時刻と一致します**が、**bisect はしていないので断定はしません**。
>
> **なぜ入ったのかは説明が付きます。**`tests.yml` は `[pull_request, workflow_dispatch]` でしか動かず、**master への push では何も走りません**。[#7237](https://github.com/duplicati/duplicati/pull/7237) で Travis のバッジを消したときに調べた穴と**同じもの**です。あのとき「`push` トリガーを足すかは CI の構成判断」としてメンテナに委ねましたが、**その穴から実際に赤が入った**のが今回です。
>
> **`DisruptionTests` の発生はまだありません。**ただし**今は Windows に別の赤がある**ので、次に「Windows だけ赤」を見たときに反射的にフレークと判断しないこと。まず**落ちたテスト名**を見ること。

> **更新履歴 (2026-09-05)**: [#7276](https://github.com/duplicati/duplicati/pull/7276) が**マージ**され、前項の「新しい問題 — master の Windows ユニットテストが赤」は**決着しました**。
>
> **赤は止まりました。**`tests.yml` の実行履歴が修正を境にきれいに分かれています — 直近10実行のうち、09-02 19:10 から 09-04 12:52 までの**8実行すべてが failure**、#7276 自身の 09-04 13:49 と、その直後に切られた `release/canary-2.4.0.100` の 16:57 が **success**。あの赤は**差分に関係なく**落ちていたので、**canary ブランチの実行が緑になったこと自体が、2.4.0.100 にこの修正が入っている証拠**になります。ただし**データ点は2つだけ**です。第三者の PR はまだ1件も走っていません（master の先頭は canary のマージで、それ以降 `tests.yml` の実行なし）。**他人の PR で緑を見るまでが本当の確認**です。
>
> **マージは無改変・レビューコメント0件**（[bd646721aaaefbb039499b3a71594cb002b0eb7a](https://github.com/duplicati/duplicati/commit/bd646721aaaefbb039499b3a71594cb002b0eb7a)）。master を取り直して、`TryGetDiskNumber`（`VssDiskWrapper.cs:326`）・`Get-CimInstance MSFT_Partition`（同 349）・例外の `Output:`（`Windows.cs:471`）・テスト5件が入っていることを確認しました。
>
> **原因の要点** — **`-ErrorAction SilentlyContinue` はエラー「メッセージ」を消すだけで、終了コードは残します。**パーティションの無いディスクは非終了エラーになり、`powershell.exe` は **stderr に何も出さずに exit 1**。CI に届くエラー本文が空だったのはこれです。
>
> **同じ書き方が他にあっても、他も壊れている証拠にはなりません。**`-ErrorAction SilentlyContinue` は3箇所にありましたが、**露出したのは1箇所だけ**でした。両方の形を実測しています。
>
> | 形 | exit |
> |---|---|
> | 失敗するコマンドレットが**最後のパイプラインの先頭**（`GetVolumesOnDiskAsync`） | **1** |
> | 失敗が途中に埋もれ、最後の文が成功（`ListPhysicalDrivesAsync`） | 0 |
>
> 触る範囲はこの実測で決めました。`ListPhysicalDrivesAsync` は**たまたま安全**なので触っていません。
>
> **`proprietary/` への外部からの貢献に前例ができました。**着手前は「この配下のコミットは全部メンテナのもので、断られる可能性がある」と見て、PR 本文に licence を承知していることを明記して判断を委ねました。結果は**無改変でマージ**です。**今後この配下であることを理由に候補を外す必要はありません。**
>
> **`tests.yml` の穴は塞がれていません。**`[pull_request, workflow_dispatch]` のままで、**master への push では何も走りません**。今回の赤が入った経路そのものなので、**同じ形の赤はまた入り得ます。**[#7237](https://github.com/duplicati/duplicati/pull/7237) で「`push` トリガーを足すかは CI の構成判断」としてメンテナに委ねたまま、動きはありません。
>
> **`DisruptionTests` の発生はまだありません。**前項に書いた「今は Windows に別の赤があるので反射的にフレークと判断しないこと」という但し書きは**外れました**が、**まず落ちたテスト名を見る**という手順自体は残します。
>
> **未マージは0件**です。
>
> **§5 付録から [#3546](https://github.com/duplicati/duplicati/issues/3546) を削除しました。**2026-09-04 の項でクローズは報告していましたが、**付録の行は「マージ待ち」のまま残っていました**。[#7185](https://github.com/duplicati/duplicati/pull/7185) はマージ済みで Issue も COMPLETED です。削除に伴い「その他 bug」66→**65 件**、付録合計 624→**623 件**に直しました。**クローズの報告と付録の掃除は別の作業です** — 次回も両方やること。
>
> なお 2026-09-01 以降に upstream でクローズされた Issue は5件（#7271, #7242, #7205, #7188, #3546）で、**本レポートに載っているのは #3546 だけ**でした。残る4件はいずれも本レポートの調査時点（2026-07-03）より後に作成された Issue です。

> **更新履歴 (2026-09-05, 2)**: 次の実装候補として、§5 付録「その他 bug」の [#3123](https://github.com/duplicati/duplicati/issues/3123)「Bad handling of "" in commandline arguments in Duplicati.WindowsService」（2018-03-24 作成、最終更新 2025-03-24、**まだ OPEN**）を**実測で確認しました**。
>
> `Utility.WrapCommandLineElement` の Windows 分岐が組み立てた文字列を、**Windows の実パーサ `CommandLineToArgvW`**（.NET が `Main(string[])` に使うもの）へ P/Invoke で通しています。argc は `argv[0]` を含む数です。
>
> | 引数 | 現行の出力 | 解析結果 | |
> |---|---|---|---|
> | `has"quote` | `"has""quote"` | argc=**2**、`[1]` = `has"quote --next=1` | **壊れる** |
> | `C:\Temp\folder\` | `"C:\Temp\folder\\"` | argc=3、`[1]` = `C:\Temp\folder\` | 正しい |
> | `C:\Temp\folder\\` | `"C:\Temp\folder\\\"` | argc=**2**、`[1]` = `C:\Temp\folder\" --next=1` | **壊れる** |
>
> **後続の引数がまるごと前の引数に飲み込まれます。**欠陥は2つです。①`"` を `""` に置換している — `CommandLineToArgvW` は引用符が出た時点で引用モードを**抜ける**ので、閉じ引用符が引用の再開として読まれます。正しい逃がし方は `\"`。②**閉じ引用符の直前のバックスラッシュを1つしか倍にしていない**（すべて倍にする必要がある）。標準的な規則に直すと4ケースとも往復することも実測済みです。
>
> 呼び出し元は `RestAPI/Runner.cs:720-735`（UI の「コマンドラインとしてエクスポート」。ソースパス・**各詳細オプションの値**・フィルタ式が通る）と `WindowsService/Program.cs:84,134`（サービスの起動コマンドライン＝ #3123 の報告そのもの）。**既存の `UnitTest/CommandLineWrappingTests.cs:49` が壊れた形を assert している**ので、修正は既存の期待値を書き換えることになります。理由（実パーサでの往復結果）を PR 本文に明示すること。

> **更新履歴 (2026-09-08)**: **5件まとめてマージ**され、Issue が1件閉じました。
>
> | PR | 内容 | 対応 Issue |
> |---|---|---|
> | [#7278](https://github.com/duplicati/duplicati/pull/7278) | Windows の引数を Windows が読み戻せる形で逃がす | [#3123](https://github.com/duplicati/duplicati/issues/3123) — **OPEN のまま** |
> | [#7279](https://github.com/duplicati/duplicati/pull/7279) | 整合性チェックを SQLite に索引を作らせずに読む | [#5976](https://github.com/duplicati/duplicati/issues/5976) の原因側を1箇所 — **OPEN のまま** |
> | [#7280](https://github.com/duplicati/duplicati/pull/7280) | 除外フォルダの下にあるソースを捨てない | [#3220](https://github.com/duplicati/duplicati/issues/3220) — **OPEN のまま** |
> | [#7281](https://github.com/duplicati/duplicati/pull/7281) | native `ijwhost.dll` を managed 参照として解決しない | 無し（ビルド警告） |
> | [#7282](https://github.com/duplicati/duplicati/pull/7282) | Google Drive の同名を全部保持し、一覧では1件にする | [#5846](https://github.com/duplicati/duplicati/issues/5846) — **COMPLETED でクローズ** |
>
> **5件とも無改変・レビューコメント0件**（[e913399dbdd5c31c448cc18cadccafacb8b74b22](https://github.com/duplicati/duplicati/commit/e913399dbdd5c31c448cc18cadccafacb8b74b22)、[75fe85af9126841af4ce398b3719d4ce3665a420](https://github.com/duplicati/duplicati/commit/75fe85af9126841af4ce398b3719d4ce3665a420)、[990bbf6297d618e4b810d5062a0a35d096727e3f](https://github.com/duplicati/duplicati/commit/990bbf6297d618e4b810d5062a0a35d096727e3f)、[dc73c6125b02cd4d64a304f912c4893146eb577c](https://github.com/duplicati/duplicati/commit/dc73c6125b02cd4d64a304f912c4893146eb577c)、[9340b9025667f36a8d3323a2ff0a1a19eb5543ed](https://github.com/duplicati/duplicati/commit/9340b9025667f36a8d3323a2ff0a1a19eb5543ed)）。master を取り直し、`EscapeWindowsCommandLineElement`（`Utility.cs`）・`mismatchedLengths`（`LocalDatabase.cs`）・`IsCutOffByFilter`（`Controller.cs`）・`DropVanaraIjwhostReference`（csproj 2件）・`OrderByDescending(createdDate)`（`GoogleDrive.cs`）と、新規テスト3ファイル（`ConsistencyCheckQueriesTests.cs`, `NestedSourceExclusionTests.cs`, `GoogleDriveDuplicateNameTests.cs`）が入っていることを確認しました。
>
> **`Fixes` を付けるかの判断を本文で委ねたら、メンテナが自分で書きました。**#7282 は再現がスタブ止まりだったので `Fixes` を付けず、「issue を完全に満たすかはそちらで判断してほしい」と本文に書きました。kenkendk はマージの1分前に **`Fixes https://github.com/duplicati/duplicati/issues/5846`** とコメントし、そのままマージして Issue も閉じています。**確信の持てない紐付けは、書かずに判断材料だけ渡せば済みます。**
>
> **残る3件は、直っても開いたままです。**#3123・#3220・#5976 は修正がマージされた今も OPEN です。**マージは Issue を閉じません** — 本文に `Fixes` が無ければ何も起きません。付録の3行は**消さずに状態を注記**しました。前回（09-05）の教訓は「クローズしたのに付録の行が残っていた」でしたが、今回は**逆向き**です — **直っていても閉じていなければ、消してはいけません。**#5846 だけを §3.4 表・§4.7 詳細・§5 付録から削除し、§3 見出し 53→**52 件**・§3.4 15→**14 件**・「その他 bug」65→**64 件**・付録合計 623→**622 件**に直しました。
>
> **前回の宿題「他人の PR で緑を見るまでが本当の確認」は済みました。**09-08 にメンテナ自身の PR が2本走り、**どちらも緑**です（[#7286](https://github.com/duplicati/duplicati/pull/7286) 08:02、`feature/add-tests-for-clean-database-start` 08:27）。[#7276](https://github.com/duplicati/duplicati/pull/7276) の修正は**私の枝の外でも効いています**。
>
> **`tests.yml` の穴はそのままです。**`on: [pull_request, workflow_dispatch]` のままで、master への push では何も走りません。**その結果、5件が同居した master に対する CI 実行はまだ1件もありません** — 直近の緑はどれも、各 PR を**個別に**、マージ前の master へ載せて測ったものです。#7279 と #7280 はどちらも `Library/Main` を触っているので、**同居を最初に測るのは次に誰かが出す PR**になります。
>
> **`backendtests.yml` は直近10実行に赤がありません**（success 9・cancelled 1、最新は 09-08 08:27）。**`DisruptionTests` の発生もまだありません。**
>
> **ビルド警告は 0 件**（0 エラー / 0 警告、20:39、master @ 1d8c743f989b）。#7281 が入る前の master では `MSB3246` が2件出ていたので、**マージ後の master で消えたことをここで確認**しました。（`mise exec dotnet -- dotnet build Duplicati.slnx -c Debug --no-incremental`、master @ 1d8c743f989b）
>
> **§3.3 の注記を訂正しました。**#6448 について「対象の `ngclient` は本チェックアウトに編集可能ソースなし」と書いていましたが、**同じ階層に `ngclient` のチェックアウトがあります**（`@duplicati/ngclient` 0.0.235、Sapling）。**読めないから外す、と書いた候補は読めました。**ただし PR の宛先は duplicati/ngclient です。
>
> **#7281 の本文に書いた ijwhost の報告には、まだ反応がありません**（コメント0件）。要点は、リリースに載る `ijwhost.dll` が **AlphaVSS パッケージ由来の 3.100.19**（.NET Core 3.1 世代）で、host pack の 10.0.x ではないこと。**壊れてはいません** — shim を3種類差し替えて実測し、C++/CLI のアセンブリはどれでもロードされ、失敗するのは管理者権限の要る VSS 本体だけでした。**直すならリポジトリ全体に `Directory.Build.targets` を置く話**になるので、提案に留めています。**再燃したら PR 本文が一次資料です。**

> **更新履歴 (2026-09-09)**: マージ1件、未マージ2件、**CI の赤2件の原因調査**、そして**付録の大掃除**。
>
> **[#7290](https://github.com/duplicati/duplicati/pull/7290) がマージ**されました（09-08 14:25、**無改変・レビューコメント0件**）。本文に書いた `Fixes` がそのまま効き、[#4230](https://github.com/duplicati/duplicati/issues/4230)（Google 共有ドライブに**個人ドライブの**クォータを報告していた件）が **COMPLETED でクローズ**。前回 #7282 では `Fixes` を付けずに出してメンテナが自分で足したので、**今回は最初から付ける**判断にしました。
>
> **未マージが2件**:
>
> - [#7293](https://github.com/duplicati/duplicati/pull/7293) — `sync` が「不明」を表す**負のクォータを満杯と読む**。`FilelistProcessor` は `>= 0` で弾いているのに、こちらは `is not null` しか見ていませんでした。Google Drive は `about` にクォータ欄が無いアカウントで `QuotaInfo(-1, -1)` を返すので、**その宛先には sync が一度も走りません**
> - [#7294](https://github.com/duplicati/duplicati/pull/7294) — **`--quota-disable` が `--quota-size` まで黙って無効化**していました。オプション自身のヘルプが「`--quota-size` は引き続き使える」と書いているのに、`CheckQuotaAsync` が先頭で return していたためです。実測は明快で、付けると errors/warnings が**1件も出ず**、外すと `Assigned quota has been exceeded` が出ます
>
> **CI の赤2件は、どちらも私の差分では起こせないものでした** → [CI-RED-2026-09-08.md](CI-RED-2026-09-08.md)。
>
> - **サーバの 500 は原因を特定しました。**`PurgeLogData` が**タイマーから、ロックを取らずに**共有の SQLite 接続でトランザクションを開きます（`Program.cs:536` / `Connection.cs:1307`）。その窓に REST の読み取りが重なると、読み取り側は `cmd.Transaction` を付けないので **Microsoft.Data.Sqlite が不一致として拒否**し、`The transaction object is not associated with the same connection object as this command` になります。同じ穴が `FixInvalidBackupId` と `DeleteFromDb` にもあります。**直すのは3箇所に `lock (m_lock)` を足すだけ**です
> - **`FilesetFilesAsync` は仮説どまり**です。repair は消えた dlist を作り直すとき**新しいファイル名（＝新しい時刻）を1秒ずつ探して**付けるので、`Thread.Sleep(2000)` しか離れていない2つの版の**並び順が入れ替わり得ます**。**ローカル1回では再現せず**（Windows、1回 4分46秒）、失敗時の dlist 名が CI ログに残らないため、次の一手は**落ちたときに名前を出す**ことです
>
> **どちらの PR も再実行で緑**になりました（#7293 は 09-08 21:59、#7294 は 22:23）。**#7290 は赤のままメンテナがマージ**しています（落ちたのは `FilesetFilesAsync`）。
>
> **付録から20行を削除しました。**閉じているのに載ったままだった Issue で、**大半が7〜8月に COMPLETED で閉じられていた**ものです。§3.3 の唯一の候補だった #6448 も含む（2026-08-24 クローズ）ため、**§3.3 は 0 件**になりました。件数は §3 見出し 52→**51**、pending user feedback 16→**15**、minor change 30→**27**、その他 bug 64→**61**、enhancement 170→**169**、ラベル無し 240→**231**、その他 55→**52**、付録合計 622→**602** に直しています。**1件ずつ気付いて消す運用では追いつきません** — 更新のたびに **open 一覧を丸ごと取って突き合わせること**。

> **更新履歴 (2026-09-11)**: マージ1件、未マージ4件、**CI の赤2件の決着**、そして **remote sync に潜んでいたデータ喪失経路**。
>
> **[#7295](https://github.com/duplicati/duplicati/pull/7295) がマージ**されました（09-11 08:59、**無改変・レビューコメント0件**、[2eb1be62fa97de250a883369ff47578e0c728115](https://github.com/duplicati/duplicati/commit/2eb1be62fa97de250a883369ff47578e0c728115)）。[CI-RED-2026-09-08.md](CI-RED-2026-09-08.md) の1件目（サーバの 500）はこれで決着です。`ReWriteAllFieldsIfEncryptionChanged` の無ロック経路は PR 本文で**報告のみ**にしてあり、メンテナからの反応はまだありません。
>
> **2件目（`FilesetFilesAsync`）は、直す前に測りました。**「repair が付け直す dlist の時刻が版の並びを入れ替える」という仮説を、CI のワークフローを `probe/**` への push で起動する形にして **ubuntu で 105 回**（単体 48 ＋ カテゴリ 20 ＋ coverage 付き 8 ＋ 20、`tests.yml` と同じ `--collect:"XPlat Code Coverage"` の全スイート 2 回、ローカル 7 回）走らせましたが、**再現 0 回**。しかも計測すると、2つの版の間隔は 18 秒 / 5 秒 / 4 秒あり、repair が動かすのは常に **+1 秒**なので、**この仮説では並びを追い越せません**。仮説は自分で潰しました。残ったのは「落ちたときに名前が残らない」ことだけなので、約束どおり [#7297](https://github.com/duplicati/duplicati/pull/7297) で **6つの主張すべてに dlist 名と DB の版一覧を出す**診断を入れました（主張を1つ故意に反転させて、文言が実際に出ることを確認済み）。**未マージ**。次に落ちた1回で答えが出ます。
>
> **未マージが4件**:
>
> - [#7297](https://github.com/duplicati/duplicati/pull/7297) — 上記の診断
> - [#7299](https://github.com/duplicati/duplicati/pull/7299) — [#7284](https://github.com/duplicati/duplicati/issues/7284)「2.4.0.0 でジョブ DB のパスが `/var/lib/Duplicati/var/lib/Duplicati/X.sqlite` になる」の修正（`Fixes` 付き）。原因は3段でした。①`DataFolderLocator.cs:137` の home 無し分岐が `Path.Combine("var","lib",appName)` ＝ **先頭の `/` が無い相対パス**を返す。②パッケージの systemd unit は `User=` も `WorkingDirectory=` も持たず、**cwd が `/` だから偶然当たっていた**。2.4 以前はその相対値をそのまま DBPath 列に保存していた。③2.4.0.0 の `ResolveDbPath` がそれを「データフォルダからの相対」と読み替えて再結合。**報告者のパスが小文字 `duplicati` ではなく大文字 `Duplicati` なのが、この分岐が作った証拠**でした。修正は分岐を root 付きにし、既存行のために「データフォルダ自身の root 抜きパスで始まる相対値は root 基準」という**ファイルシステムを見ない**規則を共有ヘルパに置いたもの。DatabaseTool の `verify` が同じ結合を手書きしていて**本物の DB を Orphaned と呼び `cleanup --force` が消しに来る**ので、そちらも同じ規則に寄せました。Linux 限定の分岐は **WSL にツリーを複製して赤→緑を実測**（修正前 6 failed、修正後 21 passed）
> - [#7300](https://github.com/duplicati/duplicati/pull/7300) — `sync` が同名の remote ファイルで `ToDictionary` の生の `ArgumentException` を投げる（[#7285](https://github.com/duplicati/duplicati/issues/7285) のコメントに 2.4.0.0 stable の実物のスタック）。バックアップ側の `FilelistProcessor` と**同じ文面・同じ helpId** で、どちら側かを添えて報告するようにした。shortcut（`--force`、空の宛先）は辞書を作らずに**名前でしか指せない操作を2回ずつ走らせる**ので、検査は一覧の直後。**#7285 本体（Drime のページング）は直していません** — 報告者がパッチを持っています
> - [#7307](https://github.com/duplicati/duplicati/pull/7307) — remote sync の「**引数が別のパラメータに束縛されている**」2件。(a) `RemoteSynchronizationHandler.cs:242` の `Select((k, v) => $"{k}={v}")` は**（要素, 添字）のオーバーロード**で、inline sync は source（＝バックアップ自身の宛先）へ `"[auth-username, bob]=0"` を渡していた。(b) `LightWeightBackendManager` を位置引数で作っていて、**5番目の `autoCreateFolders` に指数バックオフのフラグが入り**、`--auto-create-folders` は一度も渡らず、バックオフは一度も効いていなかった。**(b) はデータ喪失経路でした**: バックオフの既定が true なので source 側でもフォルダが作られ、消えている source を作る → 再一覧が空 → 空の source は**宛先の全ファイルを削除**。**実測: 宛先5ファイル → 0、終了コード 0。**inline は `Confirm: true` なので確認も出ない。[#7302](https://github.com/duplicati/duplicati/pull/7302) として出したあと branch を誤って消してしまったので、**同じ内容を最新 master に rebase して #7307 で出し直し**（interdiff 0）
>
> **教訓 — 既存テストが「バグのおかげで」通っていることがある。**#7307 では既存の3テストが、**ツールが source フォルダを勝手に作ってくれる**という今回取り除く挙動に依存して通っていました。回帰スイートが赤くなって初めて分かった形です。「どのテストが、なぜ緑なのか」は差分を入れた後の赤でしか見えないので、**カテゴリ丸ごとの回帰は省かないこと。**
>
> **教訓 — 位置引数の `int, int, bool, bool` は、入れ替わっても黙って通る。**(b) は `git log -S` で 2025 年のコミットまで遡れました。名前付き引数にしただけで二度と起きません。
>
> **報告のみ（直していない）**: ① sync が成功するたび `RecordSyncOperation` がローカル DB の `Operation` 表へ `Rsync N` 行を入れ、`Controller.cs:634` の後始末は「Timestamp が最新の Operation 行」に相乗りするので、**バックアップのログと `Result` 行が `Rsync N` の operation id に紐づく**。ただし `LogData.OperationID` を join して読む箇所が見つからず、症状を示せないので触っていません。② sync は一覧の**フォルダ項目もファイルとして GET しようとする**（`IsFolder` を見ていない）。③ `ShouldTriggerSync` は `m_dbpath` を検査しない（`RecordSyncOperation` はする）。
>
> **付録の突き合わせ**: open 一覧 **619 件**を丸ごと取り、付録の 602 行と §4 の 51 見出しをすべて照合 → **閉じているのに残っている行は 0**。件数の変更なし。
>
> **環境**: この作業の途中で SCM が Sapling から **jj（colocated）** に変わっています。Linux 限定のテストは WSL の Ubuntu にツリーを複製して本物のスイートで測れる（手順はメモリに記録）。

> **更新履歴 (2026-09-15)**: **6 件マージ**、open は 2 件、PR 待ちのブランチが 8 本、他者の PR 1 件をレビュー。
>
> **マージ**（09-14〜15、いずれも無改変）: [#7315](https://github.com/duplicati/duplicati/pull/7315)（`sync-remote-state` の値の綴り）、[#7307](https://github.com/duplicati/duplicati/pull/7307)（remote sync のバックエンド引数の束縛ずれ）、[#7309](https://github.com/duplicati/duplicati/pull/7309)（sync が一覧のフォルダ項目を無視する）、[#7310](https://github.com/duplicati/duplicati/pull/7310)（sync の rename をバックエンド経由に、比較は一時ファイルで）、[#7312](https://github.com/duplicati/duplicati/pull/7312)（sync ツールの終了コードに削除・rename の失敗を数える）、[#7314](https://github.com/duplicati/duplicati/pull/7314)（sync の primary DB を dispose）。**open**: [#7313](https://github.com/duplicati/duplicati/pull/7313)（`case-insensitive-remote`）と [#7316](https://github.com/duplicati/duplicati/pull/7316)（inventory のフォルダ一覧を索引付き・大文字小文字区別の範囲で）。いずれも §5 付録の Issue には対応しない。
>
> **PR 待ちのブランチは [PENDING-BRANCHES.md](PENDING-BRANCHES.md) に移しました。**8 本（remote sync の `mode` 単独指定、タスク状態 API、fileset の完全一致 fallback、停止が戻らない件の一機構、一覧系 4 本）を master `343d00c4` に rebase して push 済み。PR 本文の下書きは [pr-drafts/](pr-drafts/)。以後この種の記録はメモリではなくこの枝に置く。
>
> **一覧系 4 操作は 2025-04 の導入からずっと壊れていた部分がある。**`list-file-versions` は**常に空**（ハンドラが全パスに区切りを付け、DB は `FileLookup.Path` ＝ 末尾部分と比較、版時刻は epoch 秒を ticks で読んで年 0001）。既存テストが緑だった理由は 2 つ — 結果を `foreach` の中で assert するので 0 件だと何も検査しない、DB 直叩きテストが末尾部分の列そのものを手で渡していた（08-27 の教訓「シームは呼び出し元が渡す値を受け取らなければ赤→緑は何も証明しない」の再来）。ほかにルート一覧がファイルソースの名前を延ばした別ソースを落とす（`notes.txt` / `notes.txt.old` — **UI の復元ツリーに出ない**）、`list-folder-contents` の末尾区切り無しで空、`search` のフォルダ指定が直下だけ。すべて使い捨てテストで実測してから直した。
>
> **[#7318](https://github.com/duplicati/duplicati/pull/7318)（tristanXme、Drime の 2,500 件ページング天井、[#7285](https://github.com/duplicati/duplicati/issues/7285)）をレビュー。**PR の diff を master に当ててビルド 0 警告、`~Drime` 6/6 緑。設計は issue の実測と整合するが、fail-closed 設計なので**ライブ API の前提が 1 つ外れると一覧が毎回失敗**する。未確認の前提は 3 つ — `folders/{id}/count` がソフト削除済みを数えるか、ルート一覧の末尾ページの `total` が正確か（issue の表では推定値が伸びる）、フォルダ作成応答に `hash` が有るか。**fork の PR では Drime のライブテストは走らない**（自分の #7316 でも `Drime Tests: skipped`）ので、初めてライブに触れるのはマージ後、メンテナの次の PR の CI。しかもその BackendTester は 20 ファイル 1 ページで、修正の本体（20 ページ超での日付窓の再開）は通らない。指摘 6 点は利用者に渡し、投稿はしていない。
>
> **教訓 — 「テストが緑」は「テストが何かを検査している」を含意しない。**ループの中だけで assert するテスト、シームに手作りの値を渡すテストは、機能が丸ごと壊れていても緑のまま。件数の assert を 1 行足すだけで赤になった。
>
> **更新履歴 (2026-09-19)**: メンテナの直近 PR 10 本（#7319〜#7334）の CI 赤 3 件の原因調査。**1 件はテスト自身の欠陥で修正を PR に、2 件は pCloud のサービス側**。
>
> | 赤 | 出た PR | 原因 | 処置 |
> |---|---|---|---|
> | `CIFS Tests (ubuntu-latest)` | [#7333](https://github.com/duplicati/duplicati/pull/7333)（09-17）、#7308（09-11）| **テストの欠陥** — `UntilPortIsAvailable(139)` の実体は `grep -i ':0*8B' /proc/net/tcp*`。アンカー無しなので apt が使う一時ポート 0x8B00–0x8BFF (35584–35839) にも一致し、samba のインストール中に「準備完了」を返す（失敗ジョブでは起動 0.08 秒で ready、正常時は 25–35 秒） | [#7335](https://github.com/duplicati/duplicati/pull/7335) — 445 番への TCP 接続 (`bash -c '</dev/tcp/127.0.0.1/445'`) を待つ |
> | `pCloud Tests (ubuntu-latest)` | [#7332](https://github.com/duplicati/duplicati/pull/7332)（09-17）| **サービス側** — 209 KiB のアップロードが約 61 秒後に応答無しで切断 (`HttpIOException: ResponseEnded`、`HttpConnection.SendAsync` の応答行読み取りで EOF なのでクライアントのタイムアウトではない)。リトライは成功し 20 件削除して一覧は空になったのに、次の run の冒頭で**知らない名前のファイルが 1 件**現れた。切断された 1 回目のアップロードをサーバが後から確定させたと読むのが一番素直 | なし |
> | `pCloud Tests (macos-latest)` | [#7319](https://github.com/duplicati/duplicati/pull/7319)（09-15）| 同上（715 KiB、61 秒、削除後に 1 件残留。次の run の auto-clean が消した） | なし |
>
> **CIFS は wslc の `ubuntu:22.04` で再現した**: apt 実行中 0.2 秒間隔で 223 回サンプルして旧 grep が **34 回一致**（apt の接続がローカルポート 0x8B9E を保持）。新チェックは smbd 起動前に一度も通らず、起動 0.6 秒後に通過。fork の CI（`TC_CLOUD_TOKEN` にダミー値を登録して testcontainers 系ジョブを有効化、[run 35406172226](https://github.com/JamBalaya56562/duplicati/actions/runs/35406172226)）でも 32 回失敗→ready→`TestSmb` 緑。Testcontainers は現行 `develop` でも同じ grep なのでパッケージ更新では直らない。
>
> **pCloud は 09-10 にも windows で落ちている**（3 週間で 3 回、毎回 1 OS のみ、毎回「61 秒で切断→リトライ成功→ゴーストが 1 件」）。BackendTester 側で吸収する余地は無い — ゴーストは 1 回目の失敗から 40 秒以上経ってから現れ、`ListUntilFolderIsEmpty` の再確認（1+2+4 秒）では待ち切れない。製品側では同名の再アップロードで pCloud が上書きするならデータには無害だが、切断後の遅延確定は pCloud の挙動なので、ここでは記録に留める。
>
> **fork の CI に testcontainers 系ジョブ（FTP / WebDAV / SFTP / SSH / CIFS）が走るようになった**（`TC_CLOUD_TOKEN` は存在チェックのみで、コンテナは runner の Docker で動く）。以後、自分の PR でもこれらは緑を確認できる。
>
> **自分の [#7325](https://github.com/duplicati/duplicati/pull/7325) の `Unit tests (windows-latest)` 赤は PR 自身のテストの TearDown**（`dblock.zip.aes` が使用中で `DirectoryDelete` 失敗）。`uploadIgnoresCancellation=true` では放置された 2 本のアップロードを最後に Release してそのまま返っていたので、file バックエンドが**バックアップ終了後に**対象フォルダへ書き、TearDown の削除と競合する（Windows だけ開いているファイルを消せない）。ローカル実測: finally 到達時に 2 本が飛行中、約 60 ms 後に完了。修正はテスト側で「Release 後にバックエンドから抜けるまで待つ」（`4916bc3a`、5/5 緑）。本体の変更は無改変。同 PR の前回 macOS 赤は `TestMultiExceptionOnDlistAsync(False,False,3,True)`、既知の DisruptionTests フレークの 2 つ目の署名。
>
> **メンテナの [#7274](https://github.com/duplicati/duplicati/pull/7274)（Hyper-V / MSSQL を prefix 型 SourceProvider に）を巻き取り、[#7336](https://github.com/duplicati/duplicati/pull/7336) を出した。**#7274 の CI 赤は **PR 自身のテスト**: テストは `\` 固定、`IsHyperVSource` / `IsMSSQLSource` は `Path.DirectorySeparatorChar`（Linux では `/`）と比較していた（Windows の赤は当時の master の `Test_Unknown_NoPartitions_Async`、#7276 で修正済み）。Linux で実測: 旧比較 False、新比較 True。それ以外に直したもの: `%HYPERV%`/`%MSSQL%` だけのバックアップで VSS スナップショットが dispose されない（factory に `SnapshotOwningSourceProvider` を追加、file source が無いときは最初の snapshot-aware provider が所有）、Linux でもメタデータ保存オプションが立つ、`--hyperv-ignore-client-warning` の消滅、ソースピッカーのルート閲覧のたびに `HyperVNotInstalled`/`MSSQLNotInstalled` 警告（ローカルで PR 版プラグインが `MSSQLNotInstalled` を出して赤 → 修正版で緑）。設計変更（バックアップ内パスが `%HYPERV%\<guid>\C:\...` に、フィルタは仮想パスにエンジンが適用）は本文に列挙して残した。**競合**: メンテナの open [#7329](https://github.com/duplicati/duplicati/pull/7329)（`--vss-provider-id`、CI 全緑）が同じファイルを編集し `QueryHyperVGuestsInfo` / `QueryDBsInfo` に `Guid providerId` を足す → マージ後に両プロバイダとテストモックの追随が必要。
>
> **`DisruptionTests.TestMultiExceptionOnDlistAsync` が [#7233](https://github.com/duplicati/duplicati/pull/7233) の診断つきで届いた**（fork の [run 35413835443](https://github.com/JamBalaya56562/duplicati/actions/runs/35413835443)、windows、#7336 の差分は `file://` ソースの経路を変えない）。1 つ目の署名 `(True,False,3,True)`: `Expected 1 But was 3`、版は full / partial / full、**`Interrupted runs: #0 sent 1 filelist(s), none for an earlier run; #1 sent 1 filelist(s), none for an earlier run; #2 sent 1 filelist(s), none for an earlier run`** — 中断された 3 回すべてが dlist を 1 つ上げている（通る回は `sent nothing`）。2 つ目の署名 `(False,False,3,True)`: `Expected 1 But was 2`、full が 1 秒差で 2 つ、`#0 sent 1 filelist(s), none for an earlier run; #1 sent nothing; #2 sent nothing`。[DLIST-FLAKE-INVESTIGATION.md](DLIST-FLAKE-INVESTIGATION.md) が待っていた「どの中断回が上げたか」の答えがこれ。次はこのログを起点に読む。
>
> **更新履歴 (2026-09-21)**: **2 件マージ**、PR 待ちだった 7 本を全部 PR に → open は **11 件**、PR 待ちは 0 本。
>
> **マージ**（09-21、いずれも 1 コミット・無改変）: [#7316](https://github.com/duplicati/duplicati/pull/7316)（inventory のフォルダ一覧を索引付き・大文字小文字区別の範囲で、[`4f451087`](https://github.com/duplicati/duplicati/commit/4f4510873c2d)）と [#7323](https://github.com/duplicati/duplicati/pull/7323)（`GET /api/v1/task/{id}` が終わったタスクを `Running` と答える窓、[`a2c14630`](https://github.com/duplicati/duplicati/commit/a2c14630ba60)）。09-15 以降の項に書き漏らしていた分: [#7313](https://github.com/duplicati/duplicati/pull/7313)（`case-insensitive-remote`）は 09-17、[#7335](https://github.com/duplicati/duplicati/pull/7335)（CIFS テストの samba 待ち）は 09-19 にマージ済み。いずれも §5 付録の Issue には対応しない。#7323 は [#6597](https://github.com/duplicati/duplicati/issues/6597) の調査で見つけたものだが issue の race は再現できておらず `Fixes` を付けていないので、#6597 は **OPEN のまま**。
>
> **open**: [#7324](https://github.com/duplicati/duplicati/pull/7324)（remote sync の `mode` 単独指定）、[#7325](https://github.com/duplicati/duplicati/pull/7325)（停止が戻らない件の一機構）、[#7326](https://github.com/duplicati/duplicati/pull/7326)（`list-file-versions` をフルパスで）、[#7336](https://github.com/duplicati/duplicati/pull/7336)（Hyper-V / MSSQL の SourceProvider 化。競合相手の [#7329](https://github.com/duplicati/duplicati/pull/7329) もまだ open）。
>
> **ローカルの 11 ブランチを master `e771d8b1` に rebase して push（衝突なし）し、PR 待ちだった 7 本を全部 PR にした**（[PENDING-BRANCHES.md](PENDING-BRANCHES.md)、下書きは [pr-drafts/](pr-drafts/) のまま、いずれも `Fixes` 無し）: [#7341](https://github.com/duplicati/duplicati/pull/7341)（`GET /api/v1/tasks` が開始前のタスクを `Running` と報告）、[#7342](https://github.com/duplicati/duplicati/pull/7342)（fileset の完全一致が外れると全 fileset に fallback）、[#7343](https://github.com/duplicati/duplicati/pull/7343)（`purge` / `set-locks` が無い版の指定で**全版**を対象にする）、[#7344](https://github.com/duplicati/duplicati/pull/7344)（ルート一覧が `notes.txt` / `notes.txt.old` の片方を落とす）、[#7345](https://github.com/duplicati/duplicati/pull/7345)（`list-folder-contents` の末尾区切り無し）、[#7346](https://github.com/duplicati/duplicati/pull/7346)（`search` のフォルダ指定が直下だけ）、[#7347](https://github.com/duplicati/duplicati/pull/7347)（`list-folder-contents` の既定版）。ローカルでの再ビルド・再テストはしていない（利用者の指示、CI が検査）。**#7326 #7342 #7344 #7345 #7346 #7347 の 6 本は全部 `DirectListHandlerTests.cs` にテストを足す**ので、1 本マージされるごとに残りは GitHub 上で conflict になる → その都度 master に rebase して push。#7342 と #7343 も `SetLocksHandlerTests.cs` を共有。
>
> **更新履歴 (2026-09-22)**: 09-21 に出した 11 本の CI 赤は **2 種類**、1 つは自分のテストの欠陥で修正、1 つは master 側のフレーク。master が 4 マージ進んだ（[#7318](https://github.com/duplicati/duplicati/pull/7318) Drime のページング、[#7339](https://github.com/duplicati/duplicati/pull/7339) ログイン試行の throttle、[#7348](https://github.com/duplicati/duplicati/pull/7348) remote control 要求の堅牢化）ので 11 ブランチ＋`triage` を master `34f6c753` に rebase して push（衝突なし）。
>
> | 赤 | 出た PR | 原因 | 処置 |
> |---|---|---|---|
> | `Unit tests (macos-latest)` — `PurgeTesting.PurgeWithASelectionThatMatchesNothingPurgesNothingAsync` | [#7343](https://github.com/duplicati/duplicati/pull/7343) のみ | **自分のテストの欠陥。** `Unable to create a new fileset for duplicati-20260921T185943Z.dlist.zip.aes because the resulting timestamp 6:59:45 PM is larger than the next timestamp 6:59:44 PM`。purge は版を**元の時刻＋1 秒**で書き直し、それが次の版より前でなければ止まる（[PurgeFilesHandler.cs:138](https://github.com/duplicati/duplicati/blob/34f6c753a20e/Duplicati/Library/Main/Operation/PurgeFilesHandler.cs#L138)）。テストは 2 回のバックアップの間を `Task.Delay(1500)` で空けていたが、fileset 時刻は秒に丸められるので差は 1 秒か 2 秒 — macOS だけ 1 秒側に落ちた（Win / Linux は緑）。既存の `PurgeTestAsync` は 5 秒ずつ寝ている | 最初の fileset の時刻を `list-filesets` で取り、**その＋2 秒**まで待ってから 2 回目のバックアップ（差が必ず 2 秒以上）。単一コミットに畳んで push（`5977ba3b`）。Windows で実測 `PurgeWithASelection…` ＋ `SetLocksHandlerTests` 5/5 緑 |
> | `Unit tests (macos-latest)` — `RestoreTestHandlerTests.CorruptedDblockIsReportedWithPathsAndErrorExitCodeAsync` | [#7326](https://github.com/duplicati/duplicati/pull/7326)、[#7336](https://github.com/duplicati/duplicati/pull/7336)、[#7343](https://github.com/duplicati/duplicati/pull/7343)、[#7345](https://github.com/duplicati/duplicati/pull/7345) | **master 側のフレーク**（テストは [#7333](https://github.com/duplicati/duplicati/pull/7333) が 09-21 に追加）。`The damaged volume should hold data for at least one file` — `Assert.IsTrue(affected.Count > 0)` が偽。同じ base の自分の他 7 本（#7324 #7325 #7341 #7342 #7344 #7346 #7347）とメンテナの #7348 / #7339 では macOS 緑、赤の 4 本はどれも restore-test に触らない → 非決定的。**機構は Windows で再現して確認した**（下記） | なし（自分の PR の範囲外。修正案は下記、別 PR にするか利用者に確認中） |
> | `Unit tests (windows-latest)` — `StopAfterCurrentFileAsync` | メンテナの [#7348](https://github.com/duplicati/duplicati/pull/7348)（マージ済み） | 未調査、記録のみ | なし |
>
> **`CorruptedDblockIsReportedWithPathsAndErrorExitCodeAsync` のフレークの機構（検証済み、Windows で 100 回再現）。**テストは `Directory.GetFiles(TARGETFOLDER, "*.dblock*").OrderBy(x => x)` の **先頭** を壊す（[RestoreTestHandlerTests.cs:327-331](https://github.com/duplicati/duplicati/blob/34f6c753a20e/Duplicati/UnitTest/RestoreTestHandlerTests.cs#L327-L331)）が、dblock の名前は `duplicati-b<guid>.dblock.zip.aes`（[VolumeBase.cs:210](https://github.com/duplicati/duplicati/blob/34f6c753a20e/Duplicati/Library/Main/Volumes/VolumeBase.cs#L210)）で **GUID は乱数**なので、「名前順で先頭」は毎回別の dblock。一方このバックアップ（12 ファイル 1〜60 KB、`blocksize` 10 KB、`dblock-size` 100 KB）は 4〜5 本の dblock を作り、**末尾の小さな 1 本が blocklist とメタデータのブロックだけ**（1〜3 ブロック、0.8〜1.5 KB）になる回がある。その volume は `File → BlocksetEntry → Block` を辿っても 0 件なので、乱数でそれが先頭に来ると `affected.Count > 0` が偽になる。
>
> **実測**（使い捨てテスト、同じファイル生成・同じオプションでバックアップを 100 回、全 dblock の中身を DB で数えた。結果は scratchpad `probe-dblock-100rounds.txt`）: 486 volume 中 **ファイルデータを持たない volume が 17**（17 回に 1 本ずつ。中身は blocklist のみ 9、blocklist＋メタデータ 5、メタデータのみ 3）。うち **6 回は名前順で先頭** — つまり本物のテストなら **Windows でも 100 回に 6 回落ちる**。macOS で 4/11 だったのは末尾 volume の組成がタイミングで変わるため（末尾がデータブロックを 1 つ含む回も多い: `blocks=3 files=1 blocklist=2`）。
>
> **修正案（別 PR、メンテナのテスト）**: 壊す volume を名前順ではなく **DB で「ファイルデータのブロックを持つ」と分かる dblock** から選ぶ — `dblocks` を順に `GetFilesInVolumeAsync` に通し、最初に 1 件以上返った volume を `corrupted` にする（`affected` もそれ）。テストの以後の assert（`affected.Count < files.Count`、`MissingRemoteVolume`、`Expected` に volume 名）はそのまま成り立つ。製品側の「末尾に補助ブロックだけの小さな dblock ができる」は正当な挙動なので触らない。
>
> **ローカルで `dotnet test Duplicati/UnitTest/Duplicati.UnitTest.csproj` が「NUnit failed to load」で 0 件になった**（例外は adapter が飲む）。原因は `Duplicati/UnitTest/bin/Debug/net10.0/Duplicati.Library.Interface.dll` が 09-19 の古い版（`IRestoreTestResults` 無し）のままで、`GetTypes()` が `ReflectionTypeLoadException`（自作の 20 行のローダで確認）。UnitTest の bin/obj を消しても直らず、**`dotnet build Duplicati.slnx` を 1 回通したら直った**（なぜ csproj 単位のビルドが古いコピーを残すのかは未確認）。master が進んだ後に csproj 単位で `dotnet test` する前は slnx をビルドすること。
>
> **更新履歴 (2026-09-22, 2)**: フレークを直そうとして、**その下に製品側のデッドロックが居た**。ブランチ 2 本を push、PR は出していない（利用者の指示）。
>
> **[`fix/restore-test-corrupt-data-volume`](https://github.com/JamBalaya56562/duplicati/tree/fix/restore-test-corrupt-data-volume)（`5ac801de`）** — 上の項のフレーク修正。壊す volume を「DB がファイルデータを持つと言う最初の dblock」から選ぶ。下書きは [pr-drafts/restore-test-corrupt-data-volume.md](pr-drafts/restore-test-corrupt-data-volume.md)。`RestoreTestHandlerTests` 16/16 緑。
>
> **検証しようとして製品側のデッドロックに当たった。**修正版テストを 100 回ループさせたところ、開始 30 秒（1〜2 周目）で止まり 72 分無反応。フルダンプで `Commands.RestoreTest` が `Task.InternalWaitCore` で停止、別スレッドに `SharpAESCrypt.HashMismatchException`、`Restore.BlockManager` が待機中。**元のコード（`dblocks[0]`、無改変）でも 3 周目で同じ場所に落ちた**ので、私の変更とは無関係の既存バグ。相関: 停止した 2 件はどちらも「壊した volume が 1 ファイル分のデータしか持たない回」（`affected=1`）、`affected>=6` の 8 周は全部 1 秒で完走。
>
> **機構（コード読解＋ダンプ）**: 復号が失敗 → [VolumeDecryptor.cs:121-126](https://github.com/duplicati/duplicati/blob/34f6c753a20e/Duplicati/Library/Main/Operation/Restore/VolumeDecryptor.cs#L121-L126) がチャネルを retire → BlockManager の volume consumer が `RetiredException` で [`cache.CancelAll()`](https://github.com/duplicati/duplicati/blob/34f6c753a20e/Duplicati/Library/Main/Operation/Restore/BlockManager.cs#L551) を呼ぶが、これは**その時点で登録済みの待ち手だけ**を落とす。後から `GetAsync` が登録した待ち手は誰も完了させない（[BlockManager.cs:320-347](https://github.com/duplicati/duplicati/blob/34f6c753a20e/Duplicati/Library/Main/Operation/Restore/BlockManager.cs#L320-L347)、deadlock timer は警告を出すだけで待ち続ける）。`VolumeDownloader` の `catch (RetiredException)` は入力チャネルを retire しないので、遅れて来た要求は黙って受理される。
>
> **[`fix/restore-cancel-late-block-waiters`](https://github.com/JamBalaya56562/duplicati/tree/fix/restore-cancel-late-block-waiters)（`8023eb73`）— 一機構だけの修正で、完全な修正ではない。**`CancelAll` がフラグを立て、`GetAsync` は登録直後にフラグを見て自分を落とす（削除してから `TrySetException` なので二重呼び出しでも安全）。**実測: 停止が 3 周目 → 20 周目に後退した**が、なくなってはいない。残りはメタデータ経路 — `dumpasync` で `FileProcessor.RestoreMetadataAsync` → `SleepableDictionary.GetAsync` が待機、`FileLister` が `FilesToRestore` への書き込みで停止、ログでは **4 つの file processor のうち 3 つしか retire していない**。「volume が作れなければネットワーク全体を倒す」という設計変更が要るので、ここでは踏み込んでいない。下書きは [pr-drafts/restore-cancel-late-block-waiters.md](pr-drafts/restore-cancel-late-block-waiters.md)。
>
> **この停止は CI でも起きうる**（`number-of-retries=0` の restore-test に限らず、壊れた dblock を読んだ復元全般）。ジョブのタイムアウトまで無反応になるので、原因不明の CI タイムアウトを見たらこれを疑う。
>
> **教訓 — 「フレークだ」で止めると、その下の本物を見落とす。**最初は「macOS だけの非決定的な赤」に見えたが、実測すると Windows でも 6% 落ちる名前順の問題で、さらにその下に「壊れた volume で復元が永久に止まる」製品バグが居た。フレークの再現を 100 回回そうとしなければ、どちらも見つからなかった。
>
> **道具**: `dotnet-dump collect -p <pid> --type Full` → `analyze` の `clrstack -all` と `dumpasync --type Restore --coalesce`。async のデッドロックはスレッドスタックに何も出ないので、`dumpasync` が要る。
>
> **更新履歴 (2026-09-23)**: **自分の PR が 6 本マージ**（全部無改変）、open は 5 本、フレーク修正の 1 本は**メンテナが先に同じ直し方で直した**ので不要に。
>
> **マージ**（09-23）: [#7343](https://github.com/duplicati/duplicati/pull/7343)（`purge` / `set-locks` が無い版の指定で全版を対象にする — 09-22 のテスト修正込み）、[#7324](https://github.com/duplicati/duplicati/pull/7324)（remote sync の `mode` 単独指定）、[#7326](https://github.com/duplicati/duplicati/pull/7326)（`list-file-versions` をフルパスで）、[#7341](https://github.com/duplicati/duplicati/pull/7341)（タスク一覧の待機中を `Waiting` に）、[#7342](https://github.com/duplicati/duplicati/pull/7342)（fileset の完全一致が外れても fallback しない）、[#7325](https://github.com/duplicati/duplicati/pull/7325)（停止が戻らない件の一機構）。いずれも §5 付録の Issue には対応しない。#7325 は [#6461](https://github.com/duplicati/duplicati/issues/6461) の一部なので #6461 は **OPEN のまま**。マージ後にメンテナが #7325 の `UntilCancelledAsync` を `Task.WaitAsync(token)` で書き直した（[`8d866951`](https://github.com/duplicati/duplicati/commit/8d866951d94a)）— 読んだ限り挙動は同じ。
>
> **open**: [#7336](https://github.com/duplicati/duplicati/pull/7336)（Hyper-V / MSSQL、競合相手の [#7329](https://github.com/duplicati/duplicati/pull/7329) も open のまま）と一覧系の [#7344](https://github.com/duplicati/duplicati/pull/7344) [#7345](https://github.com/duplicati/duplicati/pull/7345) [#7346](https://github.com/duplicati/duplicati/pull/7346) [#7347](https://github.com/duplicati/duplicati/pull/7347)。**PR 未作成**: `fix/restore-cancel-late-block-waiters`（壊れた volume で restore が止まる件の一機構、完全な修正ではない）。全部 master `fa55d270` に rebase して push、衝突なし。
>
> **restore-test のフレークはメンテナの [#7355](https://github.com/duplicati/duplicati/pull/7355)（kenkendk、[`2886bdac`](https://github.com/duplicati/duplicati/commit/2886bdacca1e)）が直した。**直し方は 09-22 の私の `fix/restore-test-corrupt-data-volume` と同じ（DB から「ファイルデータを持つ dblock」を選ぶ）で、向こうは**ファイル数が最多の volume** を選び、`dblocks[0]` を壊す 2 か所目のテストも直している。私のブランチは rebase で conflict になったので abandon（PR は出していなかった）。**副作用として 09-22 のデッドロックは CI で出にくくなる** — 停止した 2 件はどちらも「壊した volume が 1 ファイル分のデータしか持たない回」で、最多の volume を選べばその回は来ない。ただし製品側のバグ（壊れた volume を読んだ restore が永久に止まる）はそのまま残っている。
>
> **更新履歴 (2026-09-23, 2)**: **09-22 のデッドロックを最後まで追って直し、[#7357](https://github.com/duplicati/duplicati/pull/7357) を出した。**09-22 の部分修正は「穴 3 つのうちの 1 つ」だった。
>
> **方法**: 一時的な Information 級プローブ（待ち手の登録、要求の書き込み完了、`CancelAll`、`VolumeManager` の終了）を入れ、止まりやすい条件（データを持つファイルが**最少**の dblock を壊す）で restore-test を 40 周。止まったら `dotnet-dump` の `dumpasync --type Restore --coalesce` で待っている async の連鎖を読み、ログの順序と突き合わせる。これを 5 回繰り返した。
>
> | 版 | 止まった周 | 止まっていた場所（ダンプ） |
> |---|---|---|
> | master | 3 | — |
> | ① `CancelAll` 後の待ち手も落とす（09-22 の版） | 20 / 28 | `GetAsync` が `m_volume_request.WriteAsync` で停止。ログでは `register` の後に `requested` が無く、読み手の `VolumeManager` は既に retire |
> | ＋② `VolumeManager` が `VolumeRequest` を retire、③ block handler が `RetiredException` でも `req`/`res` を retire（どちらも `Retire()`） | 17 / 6 | ②後: 6 つの `GetAsync` がまだ書き込みで停止。③後: file processor が `block_request.WriteAsync` で停止 |
> | ＋② を `RetireAsync(true)` に | 9 | file processor が `block_request.WriteAsync` で停止 |
> | ＋③ も `RetireAsync(true)` に | **40/40 完走**（14 周が 1 ファイルだけの volume） | — |
>
> **決め手は CoCoL の実験**（CoCoL 1.8.4、バッファ 2 のチャネルを満杯にして 3 つ目の書き込みを待たせ、読み手側から retire）: `Retire()` → 待っていた書き込みは**止まったまま**、新しい書き込みも**止まる**。`RetireAsync(immediate: true)` → どちらも `RetiredException`。`RunTask` を抜けるだけ → 止まったまま。**`Retire()` は「バッファを読み切ってから retire」なので、読み手の居ないバッファ付きチャネルでは永久に完了しない。**restore パイプラインの失敗時の後始末はほぼ全部 `Retire()` なので、今回直した 2 か所以外にも同じ形が残っている可能性はある（`DownloadRequest` など、未確認）。
>
> **検証**: `RestoreTestHandlerTests` ＋ `RestoreHandlerTests` ＋ `RepairWithMissingRemoteFiles` 79/79 緑（Windows、19 分）。変更 2 ファイルに新しいビルド警告なし。新しいテストは足していない — 止まるかどうかはプロセスの retire の順序次第で、テストにしても一部の回しか捕まえられないため。40 周の実行を再現手順として PR に書いた。
>
> **ついでに**: プローブを外す一括削除で、プローブを書き足していた**本物の行を 2 行消していた**（`case BlockRequest request:` と `"Volume manager retired"` のログ）。push 前の `jj diff` で削除行を全部目視して気付き、ファイルを master から戻して入れ直した。**同じ行に書き足したプローブは、行ごと消す正規表現で外してはいけない。**
>
> **更新履歴 (2026-09-24)**: 利用者の依頼で `DownloadRequest` を調べたら、**#7357 と同じ形が下流にさらに 3 か所**あった。#7357 の 1 コミットに畳んで force-push（[`8149b181`](https://github.com/JamBalaya56562/duplicati/commit/8149b1818b9d)）、PR 本文も更新。
>
> **形**: 段が止まると、そこへ書く上流の段が次の item を渡そうと待ち続ける。失敗した段が自分の入力を `Retire()`（即時でない）で retire するか、retire しないため。
>
> | 失敗させた段（再現の作り方） | 止まっていた書き手（ダンプ） | 修正 | 修正前 → 後 |
> |---|---|---|---|
> | ダウンロード（dblock を壊す。`dblock-size` 25 KB で volume 20 本、`restore-channel-buffer-size` 2、downloader 1） | `VolumeManager` が `DownloadRequest.WriteAsync` | downloader の失敗時・中断時の入力 retire を即時に | 7 周目で停止 → 20/20 |
> | 復号（壊した dblock のハッシュとサイズを DB に書き込み、ダウンロード検査を通す） | downloader が `DecryptRequest.WriteAsync` | decryptor の失敗時の入力 retire を即時に | 4 周目で停止 → 20/20（全周 `DecryptionError`） |
> | 展開（復号した zip の中身を壊して再暗号化、ハッシュを DB に） | decryptor 4 つが `VolumeResponse.WriteAsync` | `VolumeManager` の `finally` で `VolumeResponse` も即時 retire | 5 周目で停止 → 20/20（19 周 `DecompressionError`） |
>
> **バッファ 1 では再現しない**。最初はバッファ 1 で 20/20 完走して仮説が外れたように見えたが、ログでは `VolumeManager` が `DownloadRequest` で待っていて、downloader の `Retire()` で解放されていた。CoCoL の実験をやり直すと、**`RunTask` の読み手からの `Retire()` はバッファ 1 なら書き手を解放し、2 では解放しない**。既定のバッファは `ProcessorCount` なので実際には 2 以上。**再現しないときは「直っている」ではなく「条件が足りない」を先に疑う。**
>
> **パスフレーズ違いでは decryptor の経路に届かない** — restore の手前で「既存バックアップのパスフレーズ変更」エラーになる。DB の `Remotevolume.Hash`/`Size` を壊したファイルに合わせるのが、ダウンロードを通して後段で落とす方法。
>
> **直していないもの**: `VolumeDecompressor` と BlockManager の volume consumer も失敗時に入力を `Retire()` で retire するが、上の再現では止まらなかったので触っていない（PR 本文にも明記）。
>
> **検証**: 対象 3 クラス（`RestoreTestHandlerTests` / `RestoreHandlerTests` / `RepairWithMissingRemoteFiles`）79/79 緑（25 分）。変更 4 ファイルに新しい警告なし。
>
> **更新履歴 (2026-09-25)**: **2 件マージ**、open は 4 件、conflict 1 件を解消。
>
> **マージ**（09-25、どちらも無改変）: [#7357](https://github.com/duplicati/duplicati/pull/7357)（壊れた volume で restore が永久に止まる件、[`8149b181`](https://github.com/duplicati/duplicati/commit/8149b1818b9d) — 09-24 に下流 3 か所を足した版がそのまま入った）と [#7347](https://github.com/duplicati/duplicati/pull/7347)（`list-folder-contents` の既定版、[`9002e211`](https://github.com/duplicati/duplicati/commit/9002e211047f)）。同時にメンテナの [#7356](https://github.com/duplicati/duplicati/pull/7356)（誤った missing key 警告の抑止）も入った。いずれも §5 付録の Issue には対応しない。
>
> **open**: [#7336](https://github.com/duplicati/duplicati/pull/7336)（Hyper-V / MSSQL。競合相手の [#7329](https://github.com/duplicati/duplicati/pull/7329) は open のまま）と一覧系の [#7344](https://github.com/duplicati/duplicati/pull/7344) [#7345](https://github.com/duplicati/duplicati/pull/7345) [#7346](https://github.com/duplicati/duplicati/pull/7346)。4 本とも master `64adb826` に rebase して push。
>
> **#7345 が conflict**: `DirectListHandlerTests.cs` の同じ位置に、#7347 と #7345 がそれぞれテストを 1 本足していた（09-15 から「1 本マージされるごとに文面上の衝突が出る」と見込んでいた形が、一覧系 3 本目で初めて出た）。中身は独立なので**両方残して解消**。`DirectListHandlerTests` 18/18 緑（両方のテストを含む）、解消後のブランチの差分は元のコミットと同じ +35 −1。
>
> **小さな落とし穴**: Git Bash の `grep -c $'\r'` は CR を正しく数えない（LF だけのファイルで行数より多い数を返した）。改行を確かめるときは `tr -cd '\r' < file | wc -c` で CR のバイト数を数える。
>
> **更新履歴 (2026-09-25, 2)**: [#7346](https://github.com/duplicati/duplicati/pull/7346)（`search` のフォルダ指定）が**マージ**（無改変、[`df50511d`](https://github.com/duplicati/duplicati/commit/df50511dd1a7)）。残り 3 本を master `89435a56` に rebase、conflict なし。そして依頼で `VolumeDecompressor` を調べ、**#7357 の残り 2 経路**を見つけて [#7359](https://github.com/duplicati/duplicati/pull/7359) を出した。
>
> **① decompressor が 1 つだけ**: 失敗時の `Retire()`（即時でない）は、ほかの decompressor がバッファを読み切れば完了する。1 つだけだと読み切る者がいないので、`VolumeManager` が `DecompressionRequest.WriteAsync` で永久に待つ（ダンプで確認）。既定の decompressor 数は CPU 数の半分（最低 1）なので、**2〜3 コアのマシンでは普通の構成**。09-24 の展開失敗の再現は既定数で回していたので、この経路を通っていなかった。
>
> **② 下流に止められた段**: downloader・decryptor・decompressor が `catch (RetiredException)` で抜けるとき、入力を**まったく** retire していなかった（#7357 で直したのは自分が失敗した `catch (Exception)` の側だけ）。① を直したあと 19 周目で、decryptor 4 つが全部 `RetiredException` 側で抜けて、downloader が `DecryptRequest.WriteAsync` で待っていた（ダンプで確認）。通常の終了で `RetiredException` が来るのは入力が retire されて読み切られた後なので、足した retire は何もしない。
>
> | ビルド | decompressor・downloader・decryptor | 20 周の結果 |
> |---|---|---|
> | master | 1・2・4 | 1 周目で停止 |
> | ＋① | 1・2・4 | 19 周目で停止 |
> | ＋①② | 1・2・4 / 1・1・4 / 2・2・1 / 1・1・1 | すべて 20/20（80 周中 72 周が展開で失敗、2 周が復号、6 周は壊したブロックに届く前に終了） |
>
> 対象 3 クラス 79/79 緑（21 分）。**直していないもの**: BlockManager の volume consumer の失敗時、`FileProcessor` の応答チャネル — まだ `Retire()` だが、この再現では止まらなかった。
>
> **教訓 — 「既定の設定で再現しなかった」は「直った」ではない。**09-24 はワーカー数を既定のままにしていたので、1 つだけの構成で出る経路を見逃した。並列ワーカーを持つパイプラインは、**各段 1 つずつ**の構成でも回す。
>
> **作業上のメモ**: 自作の構成マトリクスのスクリプトが結果を「失敗」と報告したが、`grep -oE '成功|失敗' | tail -1` が集計行「成功! -失敗: 0」の後ろの語を拾っていただけだった。結果の判定は行全体（`成功!` / `失敗!`）で見る。
>
> **更新履歴 (2026-09-25, 3)**: 依頼で BlockManager の **volume consumer** を調べ、#7357 系の 3 つ目の経路を見つけて [#7359](https://github.com/duplicati/duplicati/pull/7359) に畳んだ。あわせて [#7345](https://github.com/duplicati/duplicati/pull/7345)（`list-folder-contents` の末尾区切り）が**マージ**（無改変、[`348bd479`](https://github.com/duplicati/duplicati/commit/348bd47942f2)）、メンテナの [#7329](https://github.com/duplicati/duplicati/pull/7329)（VSS provider の選択）と #7358 も入った。
>
> **volume consumer**: 抜け方は 2 つ。`catch (RetiredException)` は入力の `ReadAsync` からしか来ない（`cache.Set` は同期）ので、入力は retire 済みで解放する書き手はいない。問題は `catch (Exception)` — 入力を `Retire()`（即時でない）で retire していた。consumer は `DecompressedBlock` の唯一の読み手なので、decompressor が永久に待つ。**`cache.Set` を外から失敗させる方法が見つからなかった**ので、consumer に「N 個目のブロックで例外」を一時的に注入して確かめた（PR には含めていない）。修正前は 8 周目で停止、decompressor 2 つが `DecompressedBlock.WriteAsync` で待っていた（ダンプで確認）。即時 retire に直したあと、ワーカー数 3 通り × 失敗位置 3 通りの 5 構成すべて 20/20、全周が `Error` と正しく報告。**実際にこの経路に入る条件（`cache.Set` が何で失敗するか）は分かっていない** — PR 本文にもそう書いた。
>
> **squash の判断**: #7359 はレビューもコメントも付いておらず CI の失敗もなかったうえ、題名「every exit path」の範囲そのものなので、別 PR にせず畳んだ。rebase 後、ローカルのテストは回していない（利用者の指示：CI で確認）。
>
> **#7336 はもう rebase しない。**メンテナの kenkendk が、#7329 のマージ直後に**このブランチへ master をマージするコミット（[`559a8d0f`](https://github.com/JamBalaya56562/duplicati/commit/559a8d0ff0a2)）を push した**。`HyperVSourceProvider` / `MSSQLSourceProvider` / `TestHyperVSourceProvider` を 1〜3 行ずつ直していて、#7329 で増えた `providerId` への追随と読める（中身は未確認）。rebase して force-push するとメンテナのコミットを消すので、以後 #7336 は触らない。**他人が push したブランチは、rebase の前に必ずコミットの作者を確かめる**（今回は `gh pr view --json commits` で気付いた）。
>
> **更新履歴 (2026-09-26)**: 依頼で **`FileProcessor`** を調べ、#7357 系の 4 つ目の経路を見つけて [#7359](https://github.com/duplicati/duplicati/pull/7359) に畳んだ。あわせて [#7336](https://github.com/duplicati/duplicati/pull/7336) が**閉じられた**（下記）。
>
> **FileProcessor**: 1 ファイルの処理で失敗すると、FP は `block_request`（自分が書き手）と `block_response`（自分が読み手）を `Retire()` して抜ける。FP は `restore-channel-buffer-size` 個先まで要求を出しているので、相方の block handler は FP が抜けた後も応答を書き続け、読み手がいないのでバッファが溢れた時点で永久に待つ。block handler が止まると `Task.WhenAll` で BlockManager ごと止まる。volume consumer と同じく FP に「N 個目のブロック書き込みで例外」を一時的に注入して確かめた（PR には含めていない）。**FP 4・バッファ 4 で 3 周目に停止**、block handler が応答チャネルの `WriteAsync` で待っていた（ダンプで確認）。FP 2・バッファ 2、FP 1、バッファ 0 の 4 構成は修正前でも止まらなかった — 先読みが多いほど溢れやすい。修正は `block_response` の retire（中断時・失敗時・`finally` の 3 か所）を即時に。`block_request` は FP が書き手なので `Retire()` のまま（handler が残りを読み切るのが正しい）。修正後は 6 構成とも 20/20、全周の結果が `Error` で失敗ファイル数も正しい（FP 複数なら 1、FP 1 つなら 12）。
>
> **気になっていた `FilesToRestore`（バッファ 0、FP は抜けるとき retire しない）は実測で問題なし** — FP が全部抜けても `FileLister` は止まらなかった（FP 1 つの構成で 40 周）。
>
> **#7336 は閉じられた（マージではない）**。メンテナの kenkendk が「このコミットを #7274 に取り込んだ（そのブランチに rebase、作者は利用者のまま）ので、そちらに一本化する」とコメントして 09-25 14:24 に閉じた。「rebase されていたので綺麗にはマージできなかったが cherry-pick した」とのこと。こちらの対応は不要。ローカルの `feature/hyperv-mssql-source-providers` はもう使わない。
>
> **作業上の失敗 2 件**: (1) 監視スクリプトが、probe の最初の行が出る前（テスト探索と最初のバックアップで 60 秒超）を「進まない」と数えて**ハングと誤報**した → 最初の行が出るまでは 180 秒待つように直した。(2) 詳細ログの probe（20 周）が**まだ動いている間に**次の構成を始め、テスト用フォルダの取り合いで失敗した → 始める前にテストプロセスが 0 個であることを確かめる。**時間の比較**（注入時 15.7 秒 vs 注入なし 2.8 秒）はビルド直後の負荷の影響が大きく、後の周では 4〜8 秒に下がったので「待ちがある」とは結論しなかった。
>
> **更新履歴 (2026-09-26, 2)**: **カナリア 2.4.0.102 がリリースされた**（[#7361](https://github.com/duplicati/duplicati/pull/7361)、`Version bump to v2.4.0.102-2.4.0.102_canary_2026-09-25`）。前回のカナリア 2.4.0.101（09-11）以降にマージされた**利用者の PR 20 本**が入った:
>
> - **remote sync（9 本）**: [#7307](https://github.com/duplicati/duplicati/pull/7307) バックエンド引数、[#7309](https://github.com/duplicati/duplicati/pull/7309) フォルダ項目の無視、[#7310](https://github.com/duplicati/duplicati/pull/7310) rename と比較、[#7312](https://github.com/duplicati/duplicati/pull/7312) 終了コード、[#7313](https://github.com/duplicati/duplicati/pull/7313) `case-insensitive-remote`、[#7314](https://github.com/duplicati/duplicati/pull/7314) primary DB の dispose、[#7315](https://github.com/duplicati/duplicati/pull/7315) `sync-remote-state` の綴り、[#7316](https://github.com/duplicati/duplicati/pull/7316) inventory のフォルダ一覧、[#7324](https://github.com/duplicati/duplicati/pull/7324) `mode` 単独指定
> - **一覧・版選択（6 本）**: [#7326](https://github.com/duplicati/duplicati/pull/7326) `list-file-versions`、[#7345](https://github.com/duplicati/duplicati/pull/7345) 末尾区切り、[#7346](https://github.com/duplicati/duplicati/pull/7346) `search` のサブフォルダ、[#7347](https://github.com/duplicati/duplicati/pull/7347) 既定版、[#7342](https://github.com/duplicati/duplicati/pull/7342) fileset の完全一致、[#7343](https://github.com/duplicati/duplicati/pull/7343) `purge`/`set-locks` が全版に及ぶ件
> - **タスク状態 API（2 本）**: [#7323](https://github.com/duplicati/duplicati/pull/7323)、[#7341](https://github.com/duplicati/duplicati/pull/7341)
> - **停止・restore（2 本）**: [#7325](https://github.com/duplicati/duplicati/pull/7325)（停止が戻らない件の一機構）、[#7357](https://github.com/duplicati/duplicati/pull/7357)（壊れた volume で restore が止まる件）
> - **テスト**: [#7335](https://github.com/duplicati/duplicati/pull/7335)（CIFS テストの samba 待ち）
>
> **その後、利用者が** open の [#7344](https://github.com/duplicati/duplicati/pull/7344) と [#7359](https://github.com/duplicati/duplicati/pull/7359)、`triage` を master `6ed3c678` に rebase して push した。rebase の前後で差分の中身が同じ、どちらの PR も利用者の 1 コミットのまま、を確認。CI は両方とも実行中。#7336 の中身を取り込んだメンテナの [#7274](https://github.com/duplicati/duplicati/pull/7274) はまだ open なので、この版には入っていない。
>
> **更新履歴 (2026-09-26, 3)**: 次の実装候補を洗い出し、**[#3123](https://github.com/duplicati/duplicati/issues/3123) に報告者宛てのコメントを投稿**した。
>
> **実装候補**（どれも今日 OPEN、修正 PR なしを確認）: [#4159](https://github.com/duplicati/duplicati/issues/4159)（Web UI のコマンドラインから送った `delete --version=N` が、実行前に割り込んだ定期バックアップで番号がずれて**別の版を消す**。`CommandlineRunService.StartTask` が引数をそのまま積むので今も残っていると読める、再現は未確認）、**restore で 1 ファイルの書き込み失敗が file processor ごと落とす**（09-26 の注入で FP 1 つなら 12/12 失敗、Issue なし）、[#4644](https://github.com/duplicati/duplicati/issues/4644)（失敗した restore に「成功」表示。ngclient は `Status == 'Completed' && ErrorMessage == null` で成功と判定（`restore-progress.component.ts:99`）、未検証）、[#5572](https://github.com/duplicati/duplicati/issues/5572)（ライブログが行を飛ばす）、[#5573](https://github.com/duplicati/duplicati/issues/5573)（VSS でバックアップしたファイルの restore で「ロック中」警告）。
>
> **#3123 は「本当に直っているか」を確かめてからコメントした。**#7278 は本文に「報告の例は #7278 の前から正しく引用されていた、`"""..."""` の形は解析側が求める形ではない」と書いて `Fixes` を付けていなかった。そこで今の master の `WrapAsCommandLine` で報告者の引数をまとめ、サービス登録と同じ `"<exe>" SERVER ...` の形を `CommandLineToArgvW` で読み戻した（使い捨てのコンソールプログラム）: 報告の 2 つの形も、#7278 で直した種類（値の中の `"`、末尾のバックスラッシュ）も元どおり、**空の引数そのもの（`""` 単独）だけが消える**（#7278 が「手を付けていない」と書いた既知の点。オプションは必ず名前付きなので登録には影響しない）。#7278 はカナリア 2.4.0.101 から。コメントは「今の版で動く（実測）、空の値は `=` の後を空に、関連の欠陥は #7278 で修正、空の引数だけ残る、safe to close」。
>
> **今後は `Fixes #N` を付ける**（利用者の指示）。PR を出す前に症状で Issue を検索する。同じく「直ったのに OPEN」の [#3220](https://github.com/duplicati/duplicati/issues/3220)（#7280 で修正）と [#5976](https://github.com/duplicati/duplicati/issues/5976)（#7279 で一部）も、同じ確認をしてからコメントする候補。
>
> **更新履歴 (2026-09-26, 4)**: 「直ったのに OPEN」の残り 2 件、[#3220](https://github.com/duplicati/duplicati/issues/3220) と [#5976](https://github.com/duplicati/duplicati/issues/5976) も、**報告者の状況で本当に直っているかを実測してから**コメントした。
>
> **#3220**（除外したフォルダの中にあるソースが警告なしにバックアップされない）: [#7280](https://github.com/duplicati/duplicati/pull/7280) は本文に「報告者の正確な形（ドライブのルート `C:\` を含むソース）は試していない」と書いていた。そこで `Controller.ExpandInputSources` を報告の形（ソース `C:\` と `C:\Windows\Fonts\`、除外 `C:\Windows\`）で直接呼ぶ一時テストを書いた: **`Fonts` はソースに残り、除外のフィルタも同じオブジェクトのまま**。ログは今 `Keeping source "C:\Windows\Fonts\" although it is inside "C:\", because a folder between them is excluded` になる（`Controller.cs:1575`）。後半の「フィルタだけで除外フォルダの下を含めたい」という議論は #7280 と別件なので、ドキュメントなら [duplicati/documentation](https://github.com/duplicati/documentation) だと添えた。[コメント](https://github.com/duplicati/duplicati/issues/3220#issuecomment-5840644548)。
>
> **#5976**（自動索引のせいで `PRAGMA optimize` が「スキーマが変わった」で失敗する、メンテナの報告）: [#7279](https://github.com/duplicati/duplicati/pull/7279) は本文に「その因果は誰も確認していない」と書いていた。今の master で `PRAGMA optimize` は DB を閉じるたびに走り、失敗すれば例外が上がる（`LocalDatabase.cs:3477`）。`AutomaticIndexTests` と同じ SQLite のログフック（`sqlite3_config_log`）で**全メッセージをコード付きで記録**しながら、バックアップ 2 回・検証・compact・restore を回した: 残る自動索引 `B(VolumeID)` は 2 回作られた（2 回目のバックアップと compact）、**ほかのログは 0 件**（`SQLITE_SCHEMA` = 17 も無し）、ログファイルでは 5 操作すべての後に `PRAGMA optimize took ...` が出て成功。[コメント](https://github.com/duplicati/duplicati/issues/5976#issuecomment-5840692773)。「自動索引はスキーマ変更ではないので失敗させようがない」とは書かず、「今回の実行では作られた後も成功した」に留めた。
>
> **作業上のメモ**: triage の付録に #3220 の注記を足したとき、perl の置換でバックスラッシュが崩れて `C:Windowsonts` になっていた。読み返して気付き、Edit で直した。**バックスラッシュを含む文字列は perl / heredoc を通さず Write・Edit で書く**（メモリの落とし穴と同じ）。
>
> **更新履歴 (2026-09-26, 5)**: #3123 の「治りきっていない」部分を直して [#7362](https://github.com/duplicati/duplicati/pull/7362) を出した（**`Fixes #3123`** 付き。GitHub も閉じる Issue として認識）。
>
> **原因**: `Utility.WrapCommandLineElement` の先頭の `if (string.IsNullOrWhiteSpace(arg)) return arg;`。空文字は引用されずに何も足さず、空白だけの文字列は引数の区切りとして読まれるので、どちらも消える。呼び出し元は 2 つ — Windows サービスの登録（`WindowsService/Program.cs:84, :134`、空の引数が登録されたコマンドラインから落ちる）と、UI の「コマンドラインとしてエクスポート」（`Runner.cs:727`、**空白だけの値のオプションが `--name= ` になり値が消える**）。
>
> **修正**: null は null のまま、空文字は `""`、空白だけは早期 return をやめて通常の経路へ（空白は `COMMANDLINE_SAFE` の外なので引用される）。
>
> **検証**: `CommandLineWrappingTests` に 17 件追加（空・空白の文字列の形、`CommandLineToArgvW` での読み戻し、サービスの形のコマンドライン全体、エクスポートの `--name=` + 値の形、null）。直す前 13 赤 / 35 緑、直した後 48 緑。エクスポートの形は空白 1 文字の値だけが赤で、`--name=` の直後の空の値は直す前から正しかった。bash でも `""`・`" "`・`--passphrase=" "` が元どおりに読める（`printf '<%s>\n'`）。**呼び出し元への影響**: エクスポートで空の値が `--name=` から `--name=""` になる（読み戻すと同じ）。サービスには空の引数がそのまま届くようになるが、サーバーの解析は `--名前=値` しか取らず、残りはヘルプの判定にしか使わないので無害（コード読解）。PowerShell は確かめていない。
>
> **#3123 に投稿済みのコメントに誤りを見つけた。**調査中に、サーバーのオプション解析（`CommandLineParser.ExtractOptions:59-60`）が値の両端の `"` を 1 組取り除くことに気付いた。報告者の 2 つ目の形 `--webservice-sslcertificatepassword=""` は、値 `""` の引用符が外れて**空のパスワード**になる（`FilterCollector.ExtractOptions` に通して実測：長さ 0。証明書のパスも引用符が外れる）。コメントには「空ではなく引用符 2 文字のパスワードになる」と書いたので逆だった。差し替え文を利用者に提示し、許可を得て**同日にコメントを編集して訂正した**（該当段落のみ。編集前後の本文を比べて、ほかは変わっていないことを確認）。
>
> **更新履歴 (2026-09-26, 6)**: 実装候補の 1 番目、[#4159](https://github.com/duplicati/duplicati/issues/4159)（Web UI のコマンドラインから送った `delete --version=N` が別の版を消す）を直して [#7363](https://github.com/duplicati/duplicati/pull/7363) を出した（**`Fixes #4159`** 付き、GitHub も認識）。
>
> **原因**: `CommandlineRunService.StartTask` は引数をそのままキューに積む。`--version` は版の一覧（`Fileset` を時刻の新しい順）での**位置**で、`DeleteHandler` の `SpecificVersionsRemover` は実行時点の一覧から選ぶ。先に定期バックアップ（または送った時点で走っていたバックアップ）が終わると先頭に版が増え、番号が 1 つずれる。`delete` には `--time` で版を名指しする手段が無いので、番号を時刻に書き換えて渡すこともできない。
>
> **修正**: 新しい `CommandlineVersionPinning`（WebserverCore/Services）。送った時点で `--version`（`Options.Version` で解釈するので並びや範囲も同じ規則）を `--dbpath` の DB の版の時刻に固定し、実行時に今の番号へ付け替えて `--version` を書き換える。DB は**操作のロックを取らずに読み取り専用で**読む — 走っているバックアップのロックを待つと、読めるのはバックアップの後で、番号がもうずれているため。固定した版が実行までに消えていたら（保持期間による削除など）**実行せずにエラー**。送った時点で範囲外の番号は固定せず従来どおり。Web UI は `--dbpath` を必ず付ける（`commandline.component.ts:154`）。
>
> **再現と検証**: `StartTask` を偽のキュー（渡されたタスクを持っておくだけ）で呼び、間に `Controller` でバックアップを 1 回、その後キューのタスクの `Run` をリフレクションで実行（キューの実行と同じ呼び方）。直す前: `--version=1` は送った時点の版 1 を残して別の版を消す、`--version=0-1` も同様、固定した版を先に消した場合はエラーにならず実行される — 3 赤。間に何も無い場合は緑（番人）。直した後 4 緑、変更ファイルに警告なし。**対象外**: `--dbpath` の無い API 直叩き（従来どおり）、`compare` の位置引数の版番号（読むだけ）。
>
> **更新履歴 (2026-09-26, 7)**: 実装候補の 2 番目、**restore で 1 ファイルの作成・書き込みが失敗すると file processor ごと落ちる**（Issue なし）を直して [#7364](https://github.com/duplicati/duplicati/pull/7364) を出した。同じ症状の open Issue は無し（検索済み）。メタデータ側の同じ形は閉じた [#6845](https://github.com/duplicati/duplicati/issues/6845) で、#6966 が `RestoreMetadataAsync` で握って直している。
>
> **原因**: `FileProcessor` のファイル単位の `catch` が `BrokenLocalFiles` に記録したあと rethrow し、FP が終わる。FP 1 つ（`restore-file-processors` の既定は `ProcessorCount / 2`、最低 1）なら restore 全体がその例外で失敗し、後続ファイルは戻らない。止めていた理由と思われるもの: FP は `restore-channel-buffer-size` 個まで Download 要求を先に送るので、失敗時に応答が `block_response` に残っている。また BlockManager はファイルが要る全ブロック（メタデータ含む）を数えていて、使われなかった分は最後まで残る。
>
> **修正**: `RetiredException` 以外（=そのファイルだけの失敗）は記録・ログして次のファイルへ。その前に ① 送った要求数−読んだ応答数だけ応答を読み捨て ② 未解放の missing block とメタデータブロックを `CacheEvict` で解放 ③ ファイルはハンドラ内で閉じる（閉じるのに失敗しても FP を落とさない）。
>
> **再現と検証**: 新規 `RestoreFileFailureTests`（FP 1、先読み 4、ブロックはバックアップから）。①ファイルの位置にフォルダ（要求前に失敗）②DB でファイル長を 1 PiB にして `restore-preallocate-size`（先読み要求を送った直後の `SetLength` で失敗）。どちらも最大のファイルなので最初に処理され、他 10 ファイルは後に来る。直す前 2 赤（restore 全体が `UnauthorizedAccessException` / `ArgumentOutOfRangeException` で失敗）→ 直した後 2 緑、**Windows と Linux (WSL ext4) で同じ結果**。変異で確認: 読み捨てを外すと②が**ハング**（テストに 2 分の上限を付けた）、解放を外すと両方 `BlockCountError ... 21`（データ 20 + メタデータ 1）、メタデータ解放だけ外すと `1`。既存の restore 関連 21 件は緑。**対象外**: 優先ファイルの失敗は従来どおり優先バリアを fault させる、既存の `continue` 経路（対象ファイルの確認失敗・空ファイル作成失敗）はブロックを解放しない（据え置き）。#7359 と同じ `catch` の近くを触るので、後から入る方に小さな rebase が要る。
>
> **更新履歴 (2026-09-26, 8)**: 実装候補の [#4644](https://github.com/duplicati/duplicati/issues/4644)（失敗した restore に「成功」表示）を直して [#7365](https://github.com/duplicati/duplicati/pull/7365) を出した。同じ症状の [#4051](https://github.com/duplicati/duplicati/issues/4051)（2020、OPEN）も見つけたので **`Fixes #4644` と `Fixes #4051`**（GitHub も両方認識）。
>
> **原因**: 元の状況（保存先フォルダがない）は、今は `FolderMissingException` が投げられて `Failed` になる（Controller で実測、API 経由は新しい番人テストで確認）。残っているのは**例外を投げずにエラー付きで終わる restore**。読めないファイルが restore 先にあるときは、`ParsedResult=Error`・エラー 4 件・復元 0 件で正常に戻る（master で実測）。`QueueRunnerService` は `Runner.RunAsync` の結果を捨てていて、`TaskQueueService` は例外があるときしか `ErrorMessage` を入れない。そのため `Completed`・`ErrorMessage` null となり、ngclient の `restore-progress.component.ts` の `Completed && ErrorMessage == null` で「成功」になる。#7364 が入ると、ファイルが書けない場合もこの形（例外なし）になるので、あわせて必要。
>
> **修正**: `CachedTaskResult` に `ErrorMessage`（省略可）を足し、`ParsedResult` が Error/Fatal のときにエラー 1 件ならその文、複数なら `Got N error(s)`（Runner の通知と同じ）を残す。`TaskQueueService` の 3 か所で、例外がなければそれを返す。`Status` は `Completed` のまま — ngclient の `server-state.service.ts` は Completed/Failed で終了を判定するので、新しい値は足さない。ngclient の変更は不要。
>
> **検証**: `ServerApiIntegrationTests` に 2 件（サーバーを起動し API でバックアップ→restore→`/api/v1/task/{id}`）。①読めないファイルがある（Windows は共有なしで開いたまま、他 OS はパーミッションなし、読めてしまうなら Ignore）: 直す前は赤（`ErrorMessage` null）→ 直した後は緑。**Windows と Linux (WSL ext4) で実測**。②保存先を移動: master でも緑（`Failed`、番人、Windows）。クラス全体＋`TaskQueueServiceTests` 20 件が緑。**対象外**: `GetTaskQueue` 側はテストで通っていない。ngclient の他の画面（restore 画面からの DB 修復など）は `Status` だけを見るので変わらない。
>
> **更新履歴 (2026-09-26, 9)**: #7364・#7365 の実装中に気付いた点を洗い出し（未修正 5 件）、1 番「スキップしたファイルがブロック数を戻さない」を直して `fix/restore-skipped-file-releases-blocks` を push した（**PR はまだ出さない**、#7364 の上に積んだので #7364 のマージ後に出す）。
>
> **原因**: BlockManager はファイルが要る全ブロック（メタデータ含む）を数え、0 になるまでブロックとボリュームを保持する。`FileProcessor` の `continue` 経路（対象ファイルの確認失敗、ブロック数の不一致、空ファイルの作成失敗、ボリューム ID が負）は解放しないので、終了時に `BlockCountError`/`VolumeCountError` が出る。#7364 の上で実測すると、21 / 1 / 21。さらに **FileAlreadyExists**（上書きなしで別ファイルがあり、日付付きのコピーが既に正しい内容）は `missing_blocks.Clear()` するだけで 20 が残り、**問題のない restore がエラー 2 件で終わる**。
>
> **修正**: スキップ時に #7364 の `ReleaseUnusedBlocksAsync` で残りを解放。FileAlreadyExists は missing を verified に移し、既存のループで解放する。
>
> **検証**: 新規 4 件は直す前 4 赤 → 直した後 4 緑。**Windows と Linux (WSL ext4) で同じ結果**。ブロック数の不一致の経路は到達できないのでテストなし。既存の `RestoreOtherProcessIsUsingFileAsync` は「エラー 4 件」がこの副作用 2 件込みだったので、2 件（ファイルのエラー＋`RestoreFailures`）に直した。
>
> **残りの気付き**（未修正）: ② 空ファイルの作成に失敗しても `BrokenLocalFiles` に入らない（今回の実測で `broken=[]` を確認済み）、③ 優先ファイルが 1 つ失敗すると restore 全体が止まる（コードを読んだだけ）、④ 保存先がないと再試行で約 50 秒待つ（時間は実測、原因は推測）、⑤ `HttpReportStatus` の完了報告が例外だけで Completed/Failed を決める（#4644 と同じ形、コードを読んだだけ）。ngclient 側の DB 修復の判定は ngclient 側で対応（利用者が別途）。
>
> **更新履歴 (2026-09-26, 10)**: 気付き 2 番「空ファイルを作れなくても `BrokenLocalFiles` に入らない」を直して `fix/restore-report-empty-file-failure` を push した（**PR はまだ出さない**、1 番のブランチの上に積んだ）。
>
> **原因**: `BrokenLocalFiles` を使うのは最後のまとめ（`RestoreHandler.cs:969`、「Failed to restore N local files」と一覧）だけ。ファイルを諦める経路のうち、対象ファイルの確認失敗と書き込み失敗は登録するが、空ファイルの作成失敗（`CreateEmptyFile`）とブロック数の不一致（`BlockCountMismatch`）はエラーログだけで登録しない。空ファイルの catch は他と違って stop/abort も握っていた。ボリューム ID が負の経路は「警告でスキップ」の設計なので、そのままにした。
>
> **修正**: 2 か所で lock 付きで登録。空ファイルの catch に `when (!IsShutdownRequested)` を付ける（中断で失敗扱いしない、#7357 系と同じ）。
>
> **検証**: 新規 `AnEmptyFileThatCannotBeCreatedIsReported` は直す前 赤（一覧が空）→ 直した後 緑、**Windows と Linux (WSL ext4)**。同クラス 7 件・restore 関連の既存 19 件は緑。ブロック数の不一致と stop/abort のフィルタはテストなし（到達・タイミングの都合）。
>
> **新たな気付き**（未修正・コードを読んだだけ）: 空ファイルの作成失敗とボリューム ID が負の経路は優先バリアを fault させない。ファイル用の restore 先は優先ファイルを返さないが、restore コールバックモジュールは一覧を書き換えられる（`InvokePreparePriorityFilesAsync`）ので、その場合は他の処理が待ち続ける可能性がある（3 番と一緒に扱う）。`CopyOldTargetToNew` の失敗は `BrokenLocalFiles` に **lock なしで**追加している（`FileProcessor.cs` の retarget 部分、他はすべて lock 付き）。
>
> **更新履歴 (2026-09-26, 11)**: 2 番の調査中に気付いた A・B を調べて直し、push した（**PR はまだ出さない**、積み順は #7364 → skipped-file-releases-blocks → report-empty-file-failure → A → B）。
>
> **A（検証済み）**: 優先でないファイルは、優先ファイルが全部終わるか、優先ファイルの失敗でバリアが fault されるまで待つ。空ファイルの作成失敗とボリューム ID が負の経路はどちらもしないので、優先ファイルだと**中断まで待ち続ける**。master で、コールバックモジュールで優先ファイルにして測った: 両方とも 60 秒経っても終わらず（他の 5 ファイルは 0 件）、中断で終わる。対照の確認失敗（fault する経路）は、restore 全体が IOException で失敗（3 番の挙動。「失敗まで約 57 秒」はテスト全体の時間で、restore 自体は 1 秒前後 — (12) 参照）。修正は 2 経路で `FaultPriorityBarrierIfPriorityFile` を呼ぶ。新規 2 件は直す前 2 分で赤 → 直した後 数秒で緑、Win と Linux。
>
> **B（リストが壊れることは検証済み、restore 中の衝突は推測）**: FileProcessor 内の `BrokenLocalFiles.Add` でロックがないのは `CopyOldTargetToNew` の失敗だけ（`BrokenRemoteFiles` はロック付き）。`FileBackedStringList.Add` はストリーム位置と `m_buf` を共有していてスレッドセーフでない。ロックなしで 2 スレッドから同時に追加すると 20/20 回壊れた。修正は `lock (results)`。衝突は再現できないのでテストなし。
>
> **更新履歴 (2026-09-26, 12)**: (11) の「対照ケースは失敗まで約 57 秒（理由は未調査）」を調べた。**再現せず、restore の不具合ではない**。
>
> 57 秒は NUnit が表示したテスト全体の時間で、restore の時間ではなかった。あのテストはクラスで最初に走っていて、テストプロセスの初回起動コストとバックアップが含まれていた。同じ条件（優先ファイル・ロック・FP 1、master）で restore だけを Stopwatch で測ると、失敗して終わるまで 0.2〜1.7 秒。計 21 回で、15 回の繰り返しは 0.2〜1.0 秒。1 回目は利用者の別ビルド（rustc が約 6 コア使用）の負荷下で測った。時間がかかっていたのは最初のテストのバックアップ（負荷下で約 20 秒、負荷が下がって 5.8 秒、2 回目以降は約 0.7 秒）。比較として、FP 2 つ・優先でない・成功する場合も 0.3〜1.2 秒。
>
> **更新履歴 (2026-09-26, 13)**: 気付き 3 番「優先ファイルが 1 つ失敗すると restore 全体が失敗する」を調べた。**意図した設計で、不具合ではない**と判断し、修正していない（根拠はドキュメントとコード。実際に動かしての確認はなし）。
>
> - `IRestoreDestinationProvider.GetPriorityFiles` の説明では、優先ファイルは「データ本体の前に復元先の構造（ディスクのジオメトリ、パーティション表）を組み立てるメタデータ」。
> - 返すのは `proprietary/DiskImage/RestoreProvider.cs` だけ（`geometry` とパーティション情報の 2 ファイル）。他の restore 先は空を返す。DiskImage は優先ファイルが先に restore されている前提で、パーティション情報がないとパーティションへの restore は `DiskRestorePartitionInfoMissing` で失敗する。
> - バリアを fault させる処理はメンテナの `c780367c5`（Kenneth Skovhede、2026-08-21「Add partition level full disk backup」）で、コメントに「止まるのではなく本当のエラーで失敗させる」とある。
>
> 構造を作れなかったまま中身を書き進めるのは危険なので、「早めに失敗させる」のが正しい。改善するなら、生の例外（例: IOException）を「優先ファイル X を restore できず、restore を中止した」で包む程度（任意、利用者の判断待ち）。
>
> **更新履歴 (2026-09-26, 14)**: (13) の任意の改善を依頼されて実装し、`fix/restore-priority-file-failure-message` を push した（**PR はまだ出さない**、B の上に積んだ）。`FaultPriorityBarrierIfPriorityFile` が `UserInformationException`（`RestorePriorityFileFailed`）でバリアを fault させる。メッセージは「優先して restore する必要のあるファイル X を restore できなかったので restore を中止した: 元のエラー」で、元の例外は InnerException に残す。新規テストは直す前 赤（生の `UnauthorizedAccessException`）→ 直した後 緑、Windows と Linux。関連テスト 29 件が緑。
>
> **更新履歴 (2026-09-26, 15)**: 気付き 4 番「保存先がないと再試行で約 50 秒待つ」を直して `fix/missing-destination-no-retry` を push した（master 直上で独立、PR はまだ）。
>
> **原因（検証済み）**: `BackendManager.Handler.ExecuteWithRetryAsync` はすべての例外を再試行する。保存先を移動した restore は List を 6 回（1＋再試行 5）、10 秒おきに行い、51 秒で `FolderMissingException`。ローカル DB の有無は関係なし。バックアップ以外では `Controller.SetupCommonOptions` が `disable-autocreate-folder` を有効にするので、restore が空のフォルダを作る心配はない（確認済み）。ファイル系の保存先は共有が一時的に切れても同じ例外になるので、方針は利用者に選んでもらった → **バックアップ以外は即失敗、バックアップは従来どおり再試行**。
>
> **修正**: `MainAction != Backup` で `FolderMissingException` なら、その場で失敗（イベントも Failed）。新規テストは試行回数をログで数える。restore は直す前 4 回 → 直した後 1 回、バックアップの番人は両方 4 回。Windows と Linux。関連 18 件が緑。`LightWeightBackendManager` の同じループは対象外。
>
> **新たな気付き（検証済み、原因は未調査）**: 保存先がなく autocreate を無効にしたバックアップが失敗すると、**その DB ファイルが開いたまま残る**。20 秒待っても、GC 後もロックされていた。`PreBackupVerifyAsync` の catch は DB を Dispose しているので、別の所（接続プール、または backend manager 側の接続）が握っていると見ている。新しいテストは、別の DB を使って影響を避けた。
>
> **更新履歴 (2026-09-26, 16)**: (15) の新たな気付き「失敗したバックアップの DB が開いたまま残る」の原因を特定した（**未修正**）。
>
> **切り分け（検証済み）**: 残るのは**新しく作った DB でバックアップが失敗したときだけ**。新 DB で保存先なし: 残る。既存 DB で同じ: 残らない。新 DB でフォルダが作れない: 残る。再試行 0 回: 残る（再試行は無関係）。成功: 残らない。`Pooling=false` なので接続プールではない。GC 後もロックされたままなので、参照が生きている接続。
>
> **原因（検証済み）**: `SQLiteLoader` に一時的な記録（接続を開いたスタックを残し、閉じたら消す）を入れて特定した。残っている接続は、`Controller.RunActionAsync` の失敗ハンドラ（`catch (Exception ex)`、`Controller.cs:822`）が結果を書くために開いたもの: `LocalDatabase.CreateLocalDatabaseAsync(dbpath, operation: null, ...)`。operation が null だと「直前の Operation 行」を読むが、新しい DB では失敗したバックアップの Operation 行がロールバックされて空なので、`throw new Exception("LocalDatabase does not contain a previous operation.")`（`LocalDatabase.cs` の operation == null の分岐）。`CreateLocalDatabaseAsync(string path, ...)` は開いた接続（と `m_rtr`）を例外時に閉じないので、`await using` にオブジェクトが渡らず漏れる。失敗ハンドラはこの例外を `FailedWriteOperation` の警告にして握るだけ（実際に `FailedWriteOperation: LocalDatabase does not contain a previous operation.` が出ることを確認済み）。
>
> **影響（検証済み）**: 同じプロセスで同じ DB に 2 回目のバックアップは成功する（ロックで失敗はしない）。ただしファイルのハンドルがプロセス終了まで開いたままで、Windows ではその DB ファイルを削除できない（テストの TearDown が `IOException` で失敗）。サーバーで初回バックアップが失敗した後に DB を消そうとすると失敗すると思われる（推測）。
>
> **直し方の案**: `CreateLocalDatabaseAsync(string path, ...)` で、接続を作った後の初期化が失敗したら接続（とトランザクション）を閉じてから rethrow する。`operation: null` の呼び出しは `Controller` に他にもある（中断時の書き込みなど）が、同じ関数を直せば全部に効く。
>
> **更新履歴 (2026-09-26, 17)**: (16) の DB が開いたまま残る件を直して `fix/local-database-open-failure-closes-connection` を push した（master 直上で独立、PR はまだ）。`CreateLocalDatabaseAsync(string path, ...)` が初期化の例外時に、自分で作ったもの（コマンド、`m_rtr`、接続）だけを解放してから rethrow する。派生クラスの `DisposeAsync` は、初期化前のフィールドに触れるおそれがあるので呼ばない。新規テストは「初回バックアップが失敗した後も DB が開いたままか」を見る（Windows は排他で開けるか、Linux は `/proc/self/fd`）。直す前 赤 → 直した後 緑、Windows と Linux。関連 16 件が緑。4 番（`fix/missing-destination-no-retry`）の番人テストは、この件を避けるために別の DB を使っていて、両方入ったら元に戻せる。
>
> **更新履歴 (2026-09-26, 18)**: 気付き 5 番を直して `fix/http-report-status-result-errors` を push した（master 直上で独立、PR はまだ）。途中で DB の件を [#7366](https://github.com/duplicati/duplicati/pull/7366) として出した（Issue なし）。
>
> **原因（検証済み）**: `HttpReportStatus.OnOperationCompletedAsync` は `exception == null ? "Completed" : "Failed"` と `exception?.Message` だけで報告を作り、渡された `result` を見ていない（既存テストは result に null を渡している）。エラー付きで正常に戻る restore は `Completed`・`ErrorMessage` なしで送られる。#7365（サーバーのタスク状態）と同じ穴の報告側。
>
> **修正**: `ParsedResult` が Error/Fatal なら `ErrorMessage` を付ける（1 件ならそのエラーをログ行と同じ規則でパス伏せ、複数なら `Got N error(s)`）。`Status` と `IsCompleted` は変えない。新規 3 件は本物の `RestoreResults` を使う。直す前 2 赤 → 直した後 緑、警告だけの番人は両方緑。Windows と Linux。報告系 34 件が緑。
>
> **新たな気付き（コードを読んだだけ、未検証）**: `Failed` 報告は例外メッセージを**パスを伏せずに**送る（`exception?.Message` をそのまま）。例: 保存先フォルダがないときのメッセージにはパスが入る。ログ行には伏せる仕組みがあるので、揃えるかどうかはメンテナの判断になる。
>
> **更新履歴 (2026-09-26, 19)**: 4 番を [#7368](https://github.com/duplicati/duplicati/pull/7368) として出した（PR 本文の「DB が開いたまま」の注記は #7366 への参照に変更）。あわせて、(18) の気付き「`HttpReportStatus` の `Failed` 報告が例外メッセージをパスを伏せずに送る」を検証した。**再現した（未修正）**。
>
> **検証（実測）**: 届いた POST の本文を記録する小さな HTTP サーバーをテスト内に立て、`http-report-status-url` をそこに向けた。保存先がないバックアップを実際に走らせた（master）。既定（パスを許可していない）では、`status=Failed` の `errorMessage` に、ないフォルダのフルパスがそのまま入っていた。同じ報告の最近のログ行（同じ失敗の行）はパスが伏せられていた。`http-report-status-allow-paths-in-log-messages=true` にすると、どちらにもパスが入る。
>
> **他の報告モジュールとの比較（コードを読んで確認）**: `send-http-url` やメールなどの `ReportHelper` 系は、テンプレート（例外の文面を含む）を埋めた後の本文と件名全体に、既定で `SensitiveDataFilter.RedactPaths` をかける（`ReportHelper.cs:652-656`）。例外の文面をパスごと送るのは `HttpReportStatus` だけ。
>
> **直し方の案**: `OnOperationCompletedAsync` の `exception?.Message` にも、ログ行と同じ規則（`m_allowPathsInLogMessages` でなければ `RedactPaths`）をかける。#7367 と同じ箇所なので、#7367 の上に積むか、#7367 に含めるかは利用者の判断。
>
> **更新履歴 (2026-09-26, 20)**: (19) の「`Failed` 報告が例外メッセージをパスを伏せずに送る」を、依頼どおり [#7367](https://github.com/duplicati/duplicati/pull/7367) に含めて直した（1 コミットのまま `8372fe86` に畳んで push、PR 本文も更新）。例外メッセージと結果のエラーのどちらにも、ログ行と同じ規則（パスを許可していなければ `RedactPaths`）をかける。新規 2 件（失敗報告のパスを伏せる、許可時は残す）。直す前 1 赤＋許可時の番人 1 緑 → 直した後 緑。Windows と Linux。報告系 36 件が緑。
>
> **更新履歴 (2026-09-26, 21)**: 実装候補の残りのうち [#5572](https://github.com/duplicati/duplicati/issues/5572)（ライブログが行を飛ばす、ts678、2024-09）を直して `fix/live-log-poll-continues-from-oldest` を push した（master 直上で独立、PR はまだ、`Fixes #5572`）。
>
> **原因（検証済み）**: ngclient の `logs-live.state.ts` は、3 秒ごとに `getApiV1LogdataPoll({ id: 見た最大 ID, pagesize: 100 })` を呼ぶ。サーバーの `LogWriteHandler.AfterID` は 100 件を超えると新しいほうの 100 件を返していた（`843b3e4e`、2018）。クライアントはその最大 ID から続けるので、古いほうは二度と表示されない。単体テストで 250 行を流すと、100 行（151-250）しか届かなかった。
>
> **修正**: 続きの取得（`id > 0`）は古いほうから返す。最初の取得（`id <= 0`）は、開いたときに今の様子が見えるよう新しいほうのまま。ngclient は各ページを反転して前に積むので、古い順のページで順序が正しくなり、変更は不要。新規テストは直す前 赤 → 直した後 緑、最初の取得の番人は両方緑。関連 API テスト 2 件が緑。純粋なロジックの変更なので Linux での実測は省いた。**対象外**: まだ何も受け取っていないクライアント（`id=0` のまま）の最初の応答の前に 1 ページを超える行が来た場合と、5000 件のバッファより遅れた場合。
>
> **残りの候補**: [#5573](https://github.com/duplicati/duplicati/issues/5573)（VSS でバックアップしたファイルの restore で「ロック中」の警告）。動作は壊れておらず、警告の段階の好みの問題に近いので優先度は低い。
>
> **更新履歴 (2026-09-26, 22)**: 実装候補を洗い出して `IMPLEMENTATION-CANDIDATES-2026-09-26.md` にまとめた（7 つの調査エージェントで 29 件、古い UI も対象）。新しく見つかったものから着手し、2 番「修復が健全な DB をリネームする」を再現して直した（`fix/repair-abort-keeps-database`、push のみ）。
>
> **想定との違い（検証済み）**: 別の接続が書き込みロックを持っている状態で repair を実行すると、`Controller.RestoreOptionsFromExistingDatabase` が修復より先に DB に書き込み、そこで `database is locked` になる。修復のコードまで届かず、リネームも起きない。
>
> **実際の問題（検証済み）**: 中断（キャンセル）が同じ `catch (Exception)` に入り、DB が読めないものとして扱われていた。Controller で開始直後に中断すると、30 回中 9 回リネームしようとした。Linux では実際に DB が `.backup` に移り、WAL が元の名前のまま取り残された。修正はキャンセル時に投げ直すこと。直した後は 30 回中 0 回。
>
> **更新履歴 (2026-09-26, 23)**: 候補 3 番（#7366 の続き）を直して `fix/local-database-derived-setup-closes-connection` を push した（#7366 の上、PR はまだ）。基底の作成の後にも初期化をするのは、`LocalBackupDatabase` と `LocalDeleteDatabase`（compact・delete・purge）の 2 つだけ（他の作成処理は全部確認した）。
>
> **再現（検証済み）**: 準備する SQL だけが使うテーブル（`BlocklistHash`、`DuplicateBlock`）を消した DB で、Controller からバックアップとコンパクトを実行すると、DB が開いたまま残った。以後の書き込みは毎回 10 秒の busy 待ちになる。
>
> **再現しなかったこと**: 中断（キャンセル）が引き金になるのは、基底の作成から派生の準備までの間だけ。Controller で 2 ms 刻みに 25 回試して、一度も当たらなかった。
>
> **修正**: #7366 の解放処理を `ReleaseAfterFailedSetupAsync` に切り出し、2 つの派生の作成処理でも使う。新規 2 件は、直す前は赤、直した後は緑。Windows と Linux。関連 13 件が緑。
>
> **更新履歴 (2026-09-26, 24)**: 候補 1 番 [#7330](https://github.com/duplicati/duplicati/issues/7330) を直して `fix/server-port-refused-tries-next` を push した（master 直上で独立、PR はまだ、`Fixes #7330`）。
>
> **再現（検証済み）**: 排他ソケットでポートを塞ぐと、Windows では 10048（使用中）になり、すでに対応済みの経路に入るので赤にならなかった。そこで、この PC に実際にある Windows の除外範囲（`netsh interface ipv4/ipv6 show excludedportrange`、1032–1131 など）のポートを使い、実サーバーを `1032,<空き>` で起動した。Issue と同じ `IOException` → `AggregateException` → `SocketException (10013)` ×2 で停止した。Linux は特権ポート 1 で同じく赤。
>
> **修正**: 判定を `IsPortUnavailable` に切り出し、中身が全部「使用中／拒否」の AggregateException も次のポートへ進む。直した後は両 OS で緑。
>
> **別件の気付き（検証済み、既存の問題）**: 終了時に `QueueRunnerService.Terminate` が `ObjectDisposedException`（CTS が破棄済み）を出すことがある。`DuplicatiWebserver.StartAsync` の `App.RunAsync()` がサーバー停止時に DI コンテナを破棄し、`Program` の後片付けが `StopAsync` の後に `queueRunner.Terminate` を呼ぶ順番なので、タイミング次第。今回の変更と関係なく、既存のテストでも 3 回に 1 回出た。ログに出るだけで害は小さい。
>
> **更新履歴 (2026-09-26, 25)**: (24) の別件「終了時に `QueueRunnerService.Terminate` が `ObjectDisposedException`」を直して `fix/queue-runner-dispose-before-terminate` を push した（master 直上で独立、PR はまだ、同じ症状の Issue はなし）。サービスの側を、どちらの順番で呼ばれても大丈夫にした（`Dispose` も終了扱いにしてキャンセルし、破棄済みのキャンセルは飛ばす）。終了時と同じ順番（破棄→終了）で呼ぶ新規テストは、直す前 赤 → 直した後 緑。実サーバーでの競合は再現が不安定なので、この順番のテストを根拠にした。
>
> **更新履歴 (2026-09-26, 26)**: 候補 4 番 [#6837](https://github.com/duplicati/duplicati/issues/6837)（バックアップが終わっても Windows がスリープしない）を再現して直し、`fix/sleep-prevention-dedicated-thread` を push した（master 直上で独立、PR はまだ、`Fixes #6837`）。`SetThreadExecutionState` の要求はスレッドごとだが、`Task.Run` のループがスレッドプールのスレッドで要求し、Dispose したスレッドでだけ取り消していた。`CallNtPowerInformation(SystemExecutionState)` で、Dispose の 20 秒後も要求が残ることを実測（3 周とも）。専用スレッドが 1 回要求し、停止の合図で取り消してから終わる形にした。新規 `SleepPreventionTests`（Windows のみ）は直す前 赤 → 直した後 緑。
>
> **更新履歴 (2026-09-26, 27)**: 候補 5 番 [#3483](https://github.com/duplicati/duplicati/issues/3483)（古い UI の restore で、展開前のルートにチェックを付けられない）を再現して直し、`fix/restore-picker-unexpanded-root` を push した（master 直上で独立、PR はまだ、`Fixes #3483`）。`subst` の 2 ドライブとブラウザで確認し、直す前は `TypeError` で選択が空、直した後は選択され、復元まで確認した。**別件（未修正、Issue なし）**: 複数ドライブのバックアップでは、restore のルートの一覧に 1 ドライブ約 30 秒かかり、空のルートが 1 つ余計に出る。`GetLargestPrefixAsync` が読み取りを開いたまま再帰し、内側の `DROP TABLE` が SQLite のタイムアウトまで待つ。179d1ceb（Async 化）の退行。詳細は `IMPLEMENTATION-CANDIDATES-2026-09-26.md` の #3483 の項。
>
> **更新履歴 (2026-09-26, 28)**: (27) の別件（候補 5b）を直し、`fix/list-prefix-multiple-roots` を push した（master 直上で独立、PR はまだ、Issue なし）。パスを先にリストに読んでからルートごとに調べ、ルートを返したら終わる形にした（Async 化の前の同期版と同じ）。新規 `ListPrefixMultipleRootsTests`（DB のパスを 2 ドライブと UNC に書き換える）は、Windows と Linux の両方で、直す前 赤（余計な空のルート、約 1 分 40 秒）→ 直した後 緑（8〜11 秒）。新しい UI は、既定では v2 の list-folder を使うので**影響なし**（実測）。v2 を無効にしたときだけ v1 に戻り、62 秒待って空のルートも出た（実測）。
>
> **更新履歴 (2026-09-27)**: 候補 6 番 [#4405](https://github.com/duplicati/duplicati/issues/4405)（古い UI で S3 のクライアントの設定が aws に戻る）を再現して直し、`fix/s3-client-kept-on-edit` を push した（master 直上で独立、PR はまだ、`Fixes #4405`）。パーサーが読んだ値を、後から走るローダーが無条件に aws で上書きしていた。既存のジョブを開くたびに起き、変更せずに保存しても `s3-client=aws` が書かれた（ブラウザと API で確認）。未設定のときだけ既定を入れる形にした。新しい UI は影響なし（ブラウザで確認）。
>
> **更新履歴 (2026-09-27, 2)**: 候補 7 番 [#3750](https://github.com/duplicati/duplicati/issues/3750)（古い UI で S3 の独自のストレージクラスが保存されない）を再現して直し、`fix/s3-custom-storage-class` を push した（master 直上で独立、PR はまだ、`Fixes #3750`）。Custom を選ぶと選択が null になり、URL の組み立てが入力値を書かなかった。リージョンと同じ分岐を足した。直す前は保存後の URL にクラスがなく、直した後は入力値が入る（ブラウザと API で確認）。新しい UI は影響なし（ブラウザで確認）。
>
> **更新履歴 (2026-09-27, 3)**: 候補 8 番 [#4001](https://github.com/duplicati/duplicati/issues/4001)（古い UI でジョブを編集するとローカルフォルダの保存先が表示されない）を再現して直し、`fix/local-folder-shown-on-edit` を push した（master 直上で独立、PR はまだ、`Fixes #4001`）。テンプレートの `ng-init` が、保存先を読むより前に 1 回だけ表示を決めていた。ホームから編集へ移ると 3/3 回再現、直した後は 4/4 回正しく表示（ブラウザで確認）。新しい UI は影響なし（ブラウザで確認）。
...
## 3. トリアージ判定サマリー（深掘り 51件）

close 有力候補・実装候補・確認済みバグを中心にスレッドを個別精読しました。各カテゴリの表で全体像を、§4 に 1 件ごとの詳細所見と投稿文案（英語ドラフト）を掲載します。初版では 66 件を扱いましたが、その後 upstream でクローズされたものを順次削除しており、現在の掲載は 53 件です。

### 3.1 即 close 可能（コメントして閉じる）── 33 件
...
### 3.2 現行版/新 UI で要再検証（リテスト依頼して当面 keep）── 4 件
...
（唯一の候補だった [#6448](https://github.com/duplicati/duplicati/issues/6448) は 2026-08-24 に COMPLETED でクローズされました。**この節は空です** — 実装候補は §3.4 と §5 付録から選んでいます。）

### 3.4 維持推奨（有効なバグ・要望。将来の実装対象）── 14 件
...
## 5. Appendix — Open Issue 全 602 件のバケット別一覧
...
### pending user feedback (返信待ち→close候補) (15件)
...
| [3123](https://github.com/duplicati/duplicati/issues/3123) | 2018-03-24 | 2025-03-24 | 3 | bug | Bad handling of "" in commandline arguments in Duplicati.WindowsService — **修正マージ済み（[#7278](https://github.com/duplicati/duplicati/pull/7278)、2026-09-08）。Issue は OPEN のまま。2026-09-26 に報告者宛てに「今の版で動く（実測）、safe to close」と[コメント](https://github.com/duplicati/duplicati/issues/3123#issuecomment-5840533795)** |
| [3375](https://github.com/duplicati/duplicati/issues/3375) | 2018-09-11 | 2025-03-24 | 2 | bug | Repair never ends |
| [3536](https://github.com/duplicati/duplicati/issues/3536) | 2018-12-11 | 2025-03-24 | 22 | bug,core logic | Recursive Folders (due to symlinks?) in Linux — **調査済み・着手は非推奨**（下記 2026-08-22 の注記） |
...
| [5976](https://github.com/duplicati/duplicati/issues/5976) | 2025-02-14 | 2025-03-24 | 2 | bug | Failure messages with `PRAGMA optimize` — **[#7279](https://github.com/duplicati/duplicati/pull/7279) で自動索引を作らせる問合せ側を1箇所つぶした（2026-09-08）。warning 自体の判定は §3.1 のとおり。Issue は OPEN のまま。2026-09-26 に「残る自動索引 `B(VolumeID)` が作られても `PRAGMA optimize` は毎回成功（実測）、safe to close」と[コメント](https://github.com/duplicati/duplicati/issues/5976#issuecomment-5840692773)** |
...
| [3220](https://github.com/duplicati/duplicati/issues/3220) | 2018-05-15 | 2024-01-24 | 6 | - | "Removing source Foo because it is a subfolder" causing files to not be backed u — **修正マージ済み（[#7280](https://github.com/duplicati/duplicati/pull/7280)、2026-09-08）。Issue は OPEN のまま。2026-09-26 に「報告の形（`C:\` と `C:\Windows\Fonts\`、`C:\Windows\` を除外）で Fonts がソースに残る（実測）、safe to close」と[コメント](https://github.com/duplicati/duplicati/issues/3220#issuecomment-5840644548)** |
